...
3. **온라인 대전 6종(하나미코지·뱅!·그리드 포커·아발론·노땡스·페루도)만 예외** — Realtime Presence/Broadcast가 곧 통신 수단 자체이므로 이 게임들은 Supabase 없이 원천적으로 불가능하다. 나머지(대시보드, 내기 정산, 기록 열람)는 Supabase 유무와 무관하게 100% 동작한다. 자세한 내용은 [cloud-sync.md](./cloud-sync.md).
...
**왜**: 보드 컴포넌트를 내기 없는 단독 페이지나 다른 정산 방식에도 재사용할 수 있게 하려는 의도다. 실제로 온라인 대전 6종은 각자 다른 방식으로 `rankings`를 만들어내지만(하나미코지·그리드 포커·노땡스는 승자/점수 비교, 뱅!은 팀 승리, 아발론은 진영 승리, 페루도는 탈락 역순 생존 순서), 상위 계층은 이 차이를 전혀 몰라도 된다.

### 1.6 순수 엔진 + 제어 컴포넌트 + 얇은 네트워크 어댑터 3계층

6개의 실제 플레이 가능한 게임(하나미코지·뱅!·그리드 포커·아발론·노땡스·페루도)이 전부 동일한 3계층 구조를 따른다:
...
| 아발론 | 상대 비밀 역할 | **게임이 끝날 때까지 아무도 공개되지 않음** — 6종 중 가장 고위험 |
...
| 노땡스 | 상대 칩 개수 + 아직 안 뒤집힌 덱 순서·제외된 9장(미래 정보) | 6종 중 새는 정보가 가장 적다(비밀 역할 없음). 오히려 이 트레이드오프를 UX 기능으로 승격시킨 유일한 게임 — 방장이 "공개 모드"를 고르면 애초에 칩 개수가 공식적으로도 전원 공개되고, 콘솔에 `노땡스()`를 치면 덱 순서까지 확인할 수 있는 디버그 창구를 의도적으로 노출해둠(`src/games/no-thanks/NoThanksBoard.tsx`) |
...
이 표는 향후 "정말 부정행위 방지가 필요해지면 어디부터 서버 권위로 옮길지" 우선순위를 정할 때 참고하라고 남겨둔 것이다(아발론 > 뱅! > 그리드 포커 > 페루도 > 노땡스 > 하나미코지 순 — 노땡스는 "숨겨야 할 게 있다"는 전제 자체를 부분적으로 포기하는 방향으로 대응했으므로 굳이 서버 권위로 옮길 우선순위가 낮다).
...
- **`playable: false`인 항목은 UI에서 완전히 동일하게 취급되지만 실제 구현이 없다** — 지금 20종 중 6종만 `playable: true`다. 이는 버그가 아니라 "카탈로그가 실제 구현 개수보다 먼저 커질 수 있다"는 설계 그 자체를 보여주는 의도된 상태.
...
- `supportsAutoRanking`: 게임 엔진이 스스로 순위를 계산해 낼 수 있는지(현재 6종 모두 `true`). `false`인 게임은 `RoundResultEntry` 화면이 자동으로 뜨는 수동 순위 입력 경로로 빠진다 — 향후 순위를 스스로 못 정하는 게임(예: 협력 게임)을 추가해도 내기 정산 파이프라인이 깨지지 않도록 처음부터 분기해둔 것.
...
boardGameRule/                 게임별 공식 룰 원문 마크다운 (Avalon.md, bang.md, Grid Poker.md, noThanks.md, Perudo.md) —
