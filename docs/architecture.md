...
3. **온라인 대전 7종(하나미코지·뱅!·그리드 포커·아발론·노땡스·페루도·센추리)만 예외** — Realtime Presence/Broadcast가 곧 통신 수단 자체이므로 이 게임들은 Supabase 없이 원천적으로 불가능하다. 나머지(대시보드, 내기 정산, 기록 열람)는 Supabase 유무와 무관하게 100% 동작한다. 자세한 내용은 [cloud-sync.md](./cloud-sync.md).
...
7개의 실제 플레이 가능한 게임(하나미코지·뱅!·그리드 포커·아발론·노땡스·페루도·센추리)이 전부 동일한 3계층 구조를 따른다:
...
6개 온라인 게임 모두 **락스텝(lockstep) + 전체 상태 공유** 방식을 쓴다 — 서버 권위(authoritative server) 엔진이 없다.
...
| 아발론 | 상대 비밀 역할 | **게임이 끝날 때까지 아무도 공개되지 않음** — 7종 중 가장 고위험 |
...
| 센추리 | 상대 손패(공개 시장에서 뽑은 카드, 비밀 역할 아님) + 자원 수 | 7종 중 새는 정보가 가장 적다 — 손패 카드 자체가 시장에 공개되어 있던 카드라 "무엇을 들고 있는지"가 상대에게 알려져도 큰 전략 정보가 아니고, 자원 수치는 노땡스의 칩 개수와 비슷한 수준의 저위험 숫자 정보다 |

이 표는 향후 "정말 부정행위 방지가 필요해지면 어디부터 서버 권위로 옮길지" 우선순위를 정할 때 참고하라고 남겨둔 것이다(아발론 > 뱅! > 그리드 포커 > 페루도 > 노땡스 ≈ 센추리 > 하나미코지 순 — 노땡스·센추리는 "숨겨야 할 게 있다"는 전제 자체가 약해 굳이 서버 권위로 옮길 우선순위가 낮다).
...
- **`playable: false`인 항목은 UI에서 완전히 동일하게 취급되지만 실제 구현이 없다** — 지금 21종 중 7종만 `playable: true`다. 이는 버그가 아니라 "카탈로그가 실제 구현 개수보다 먼저 커질 수 있다"는 설계 그 자체를 보여주는 의도된 상태.
...
- `supportsAutoRanking`: 게임 엔진이 스스로 순위를 계산해 낼 수 있는지(현재 7종 모두 `true`). `false`인 게임은 `RoundResultEntry` 화면이 자동으로 뜨는 수동 순위 입력 경로로 빠진다 — 향후 순위를 스스로 못 정하는 게임(예: 협력 게임)을 추가해도 내기 정산 파이프라인이 깨지지 않도록 처음부터 분기해둔 것.
...
boardGameRule/                 게임별 공식 룰 원문 마크다운 (Avalon.md, bang.md, Grid Poker.md, noThanks.md, Perudo.md, Century.md) —
