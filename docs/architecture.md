...
> **신규 게임을 추가하려면 이 문서 대신 루트의 [ARCHITECTURE.md](../ARCHITECTURE.md)(표준 계약)를 먼저 볼 것.** 이 문서는 "무엇이 있는가"보다 "왜 이렇게 만들었는가"에 초점을 둔 심화 설계 노트(트레이드오프, 데이터 모델, 바로잡은 스펙 해석)이며, 코드가 지켜야 할 규격 자체는 [ARCHITECTURE.md](../ARCHITECTURE.md)가 단일 출처다.

관련 문서: [ARCHITECTURE.md](../ARCHITECTURE.md)(모듈형 게임 플러그인 표준) · [cloud-sync.md](./cloud-sync.md)(실시간 동기화 상세) · [features.md](./features.md)(기능별 동작) · [troubleshooting.md](./troubleshooting.md)(발견된 버그와 교훈)
...
3. **온라인 대전 10종(하나미코지·뱅!·그리드 포커·아발론·노땡스·페루도·센추리·틀린 그림 찾기·스플렌더·오이 다섯 개)만 예외** — Realtime Presence/Broadcast가 곧 통신 수단 자체이므로 이 게임들은 Supabase 없이 원천적으로 불가능하다. 나머지(대시보드, 내기 정산, 기록 열람)는 Supabase 유무와 무관하게 100% 동작한다. 자세한 내용은 [cloud-sync.md](./cloud-sync.md).
...
**왜**: 보드 컴포넌트를 내기 없는 단독 페이지나 다른 정산 방식에도 재사용할 수 있게 하려는 의도다. 실제로 온라인 대전 7종은 각자 다른 방식으로 `rankings`를 만들어내지만(하나미코지·그리드 포커·노땡스는 승자/점수 비교, 뱅!은 팀 승리, 아발론은 진영 승리, 페루도는 탈락 역순 생존 순서, 센추리는 최종 점수 비교), 상위 계층은 이 차이를 전혀 몰라도 된다.

### 1.6 순수 엔진 + 제어 컴포넌트 + 얇은 네트워크 어댑터 3계층

10개의 실제 플레이 가능한 게임이 전부 동일한 3계층 구조를 따른다(이 구조를 표준으로 강제하는 문서는 [ARCHITECTURE.md](../ARCHITECTURE.md)):
...
10개 온라인 게임 모두 **락스텝(lockstep) + 전체 상태 공유** 방식을 쓴다 — 서버 권위(authoritative server) 엔진이 없다.
...
| 아발론 | 상대 비밀 역할 | **게임이 끝날 때까지 아무도 공개되지 않음** — 10종 중 가장 고위험 |
...
| 노땡스 | 상대 칩 개수 + 아직 안 뒤집힌 덱 순서·제외된 9장(미래 정보) | 비밀 역할이 없어 새는 정보가 적은 편. 오히려 이 트레이드오프를 UX 기능으로 승격시킨 유일한 게임 — 방장이 "공개 모드"를 고르면 애초에 칩 개수가 공식적으로도 전원 공개되고, 콘솔에 `노땡스()`를 치면 덱 순서까지 확인할 수 있는 디버그 창구를 의도적으로 노출해둠(`src/games/no-thanks/NoThanksBoard.tsx`) |
| 페루도 | 상대의 주사위 눈(라운드 내내 유지, 매 라운드 재추첨) | 비밀 역할은 없지만 상대 주사위 값은 그 라운드의 판단 근거 자체라 노땡스보다는 민감하다 — 다만 아발론·뱅!의 "게임 끝까지/죽을 때까지 유지되는 정체"에 비하면 매 라운드(짧으면 수십 초) 리셋되는 정보라 상대적으로 저위험 |
| 센추리 | 상대 손패(공개 시장에서 뽑은 카드, 비밀 역할 아님) + 자원 수 | 새는 정보가 가장 적은 축이다 — 손패 카드 자체가 시장에 공개되어 있던 카드라 "무엇을 들고 있는지"가 상대에게 알려져도 큰 전략 정보가 아니고, 자원 수치는 노땡스의 칩 개수와 비슷한 수준의 저위험 숫자 정보다 |
| 스플렌더 | 상대가 예약한 카드의 정체(실물 룰상 뒷면으로만 공개되어야 함) | 예약 카드 수 자체는 항상 공개 정보라 새는 건 "어떤 카드인지"뿐 — 센추리와 비슷한 저위험대 |
| 틀린 그림 찾기 | 상대(반대 팀)가 아직 못 찾은 정답 좌표 | 애초에 규칙상 숨겨야 할 비밀 역할/손패 개념 자체가 없는 게임이라 새는 정보의 전략적 가치가 낮다 — 최저위험군 |
| 오이 다섯 개 | 상대 손패(트릭테이킹 카드 한 장 단위) | 하나미코지와 같은 성격 — 순간적인 카드 정보라 상대적으로 저위험 |

이 표는 향후 "정말 부정행위 방지가 필요해지면 어디부터 서버 권위로 옮길지" 우선순위를 정할 때 참고하라고 남겨둔 것이다(아발론 > 뱅! > 그리드 포커 > 페루도 > 노땡스 ≈ 센추리 ≈ 스플렌더 > 하나미코지 ≈ 오이 다섯 개 > 틀린 그림 찾기 순 — 뒤쪽일수록 "애초에 숨겨야 할 게 있다"는 전제 자체가 약해 서버 권위로 옮길 우선순위가 낮다).
...
- **`playable: false`인 항목은 UI에서 완전히 동일하게 취급되지만 실제 구현이 없다** — 지금 23종 중 10종만 `playable: true`다. 이는 버그가 아니라 "카탈로그가 실제 구현 개수보다 먼저 커질 수 있다"는 설계 그 자체를 보여주는 의도된 상태.
- 실제 구현을 추가하는 4단계는 [ARCHITECTURE.md §3](../ARCHITECTURE.md#3-게임-등록-절차-4단계)에 표준으로 정리되어 있다. 핵심은 `playableGames.tsx`의 `PLAYABLE_GAME_COMPONENTS`에 **동적 import**로 등록하는 단계다. `dynamic(() => import(...), { ssr: false })`를 쓰는 이유는 **대시보드 번들이 구현된 게임 수에 비례해 커지지 않게** 하기 위함이다. 게임이 100종으로 늘어도 대시보드 첫 로딩은 안 무거워진다.
- `supportsAutoRanking`: 게임 엔진이 스스로 순위를 계산해 낼 수 있는지(현재 10종 모두 `true`). `false`인 게임은 `RoundResultEntry` 화면이 자동으로 뜨는 수동 순위 입력 경로로 빠진다 — 향후 순위를 스스로 못 정하는 게임(예: 협력 게임)을 추가해도 내기 정산 파이프라인이 깨지지 않도록 처음부터 분기해둔 것.
...
src/
  ...
  games/                       게임 카탈로그 + 각 게임의 자기완결 모듈
    ...
    <game-id>/                   게임마다 동일한 표준 레이아웃 — 정확한 파일 목록·계약은
                                  [ARCHITECTURE.md §2](../ARCHITECTURE.md#2-표준-모듈-레이아웃) 참고
...
boardGameRule/                 게임별 공식 룰 원문 마크다운 — 엔진 구현의 근거 자료. 규칙 정확성을 재검증할 때
                                 이 파일과 diff할 것. 파일명을 gameId.md kebab-case로 통일하는 작업은
                                 ARCHITECTURE.md §5에 목표로 문서화되어 있으나 아직 미적용(HANDOFF.md 참고).
