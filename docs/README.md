...
| 테스트 | Vitest, **189개 전부 통과** (게임 엔진 5종 유닛 테스트만 — UI 컴포넌트 테스트 없음, 아래 "기억할 것 하나" 참고) |
| 실제 플레이 가능 게임 | 하나미코지 · 뱅! · 그리드 포커 · 아발론 · 노땡스(No Thanks!) (총 19종 카탈로그 중 5종, 나머지는 "준비중") |
...
| [cloud-sync.md](./cloud-sync.md) | 5개 온라인 대전 게임이 공유하는 락스텝(lockstep) 동기화 패턴, 방 생명주기, 재접속(`state-request`/`state-sync`), 좌석 충돌 자가치유 |
...
| [features.md](./features.md) | 대시보드/카탈로그, 내기 정산 시스템, 플레이어 식별, 5개 게임 각각의 룰 해석 판단 기록 |
