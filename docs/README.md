...
> 새 세션을 시작한다면: 먼저 저장소 루트의 **`HANDOFF.md`**(직전 세션 인수인계, 살아있는 문서)를 읽고, 이 폴더는 그보다 안정적인 배경 지식이 필요할 때 참조할 것.
...
| **클라우드 백엔드(선택)** | Supabase — Realtime(Broadcast/Presence)으로 4개 온라인 대전 게임 필수, Postgres 2테이블로 기기 간 식별 매칭·내기 기록 백업(선택) |
...
| 테스트 | Vitest, **134개 전부 통과** (게임 엔진 4종 유닛 테스트만 — UI 컴포넌트 테스트 없음, 아래 "기억할 것 하나" 참고) |
| 실제 플레이 가능 게임 | 하나미코지 · 뱅! · 그리드 포커 · 아발론 (총 14종 카탈로그 중 4종, 나머지는 "준비중") |
...
| [cloud-sync.md](./cloud-sync.md) | 4개 온라인 대전 게임이 공유하는 락스텝(lockstep) 동기화 패턴, 방 생명주기, 재접속(`state-request`/`state-sync`), 좌석 충돌 자가치유 |
| [troubleshooting.md](./troubleshooting.md) | 실제로 발생했던 버그 6건 — 증상/원인/해결법/교훈/검증 방법. "알려진 사각지대" 포함 |
...
| [features.md](./features.md) | 대시보드/카탈로그, 내기 정산 시스템, 플레이어 식별, 4개 게임 각각의 룰 해석 판단 기록 |

루트 문서(이 폴더 밖):

- `README.md` — 사용자용 설치·실행·기능 안내(한국어, 이 프로젝트의 "제품 설명서")
- `HANDOFF.md` — 세션 간 인수인계 문서, **매 세션마다 갱신되는 살아있는 문서**(이 `docs/` 폴더는 상대적으로 안정적인 배경 지식만 다룸)
- `boardGameRule/*.md` — 게임별 공식 룰 원문(Avalon.md, bang.md, Grid Poker.md) — 엔진 구현의 근거 자료
...
4개 게임 전부 `engine.ts`(순수 리듀서)만 Vitest로 테스트하고, React 컴포넌트를 실제로 렌더링해 상호작용을 검증하는 인프라(jsdom, `@testing-library/react`)는 이 저장소에 아예 설치되어 있지 않다(`vitest.config.mts`가 `environment: "node"`). 그래서 "엔진은 100% 정상, UI 레이어가 그 값을 잘못 소비"하는 버그가 자동 테스트를 전부 통과한 채로 프로덕션에 배포될 수 있다 — 실제로 이 패턴의 버그가 **두 번** 발생했다: 하나미코지의 경쟁 액션 제출 버튼이 영원히 비활성화된 사례, 그리고 뱅!의 손패 제한 로직이 액션 페이즈 내내 모든 카드 사용을 막아버린 사례(둘 다 [troubleshooting.md](./troubleshooting.md) 참고).
