...
| **클라우드 백엔드(선택)** | Supabase — Realtime(Broadcast/Presence)으로 6개 온라인 대전 게임 필수, Postgres 2테이블로 기기 간 식별 매칭·내기 기록 백업(선택) |
...
| 테스트 | Vitest, **227개 전부 통과** (게임 엔진 6종 유닛 테스트만 — UI 컴포넌트 테스트 없음, 아래 "기억할 것 하나" 참고) |
| 실제 플레이 가능 게임 | 하나미코지 · 뱅! · 그리드 포커 · 아발론 · 노땡스(No Thanks!) · 페루도(Perudo) (총 20종 카탈로그 중 6종, 나머지는 "준비중") |
...
| [cloud-sync.md](./cloud-sync.md) | 6개 온라인 대전 게임이 공유하는 락스텝(lockstep) 동기화 패턴, 방 생명주기, 재접속(`state-request`/`state-sync`), 좌석 충돌 자가치유 |
| [troubleshooting.md](./troubleshooting.md) | 실제로 발생했던 버그 7건 — 증상/원인/해결법/교훈/검증 방법. "알려진 사각지대" 포함 |
...
| [features.md](./features.md) | 대시보드/카탈로그, 내기 정산 시스템, 플레이어 식별, 6개 게임 각각의 룰 해석 판단 기록 |
...
- `boardGameRule/*.md` — 게임별 공식 룰 원문(Avalon.md, bang.md, Grid Poker.md, noThanks.md) — 엔진 구현의 근거 자료
