...
| **클라우드 백엔드(선택)** | Supabase — Realtime(Broadcast/Presence)으로 온라인 대전 게임 대부분에 필수, Postgres 테이블로 기기 간 식별 매칭·내기 기록 백업·`active_rooms`(선택) |
...
| 테스트 | Vitest — 게임 엔진 등 52개 테스트 파일(UI 컴포넌트 테스트 없음, 아래 "기억할 것 하나" 참고). 정확한 케이스 개수는 `npx vitest run` 실행 결과 참고(세션마다 갱신되므로 이 문서엔 고정 숫자를 적지 않음) |
| 실제 플레이 가능 게임 | `src/games/registry.ts`의 `GAME_REGISTRY` 43종 중 29종(`playableGames.tsx`에 동적 등록된 목록이 정확한 최신값 — 이 표엔 고정 목록을 적지 않음, 나머지는 "준비중") |
...
| [troubleshooting.md](./troubleshooting.md) | 실제로 발생했던 버그 사례 — 증상/원인/해결법/교훈/검증 방법. "알려진 사각지대" 포함 |
| [history.md](./history.md) | **시간순** 프로젝트 히스토리 — Phase 1~28은 마일스톤별 정리본, Phase 29+는 2026-09-19 문서 정리 세션에서 `HANDOFF.md`(2026-08-14~09-12분)를 한 번에 이관한 원문 아카이브(최신이 위, 나머지 Phase와 정렬 방향 반대). troubleshooting.md/architecture.md와 달리 연대기 순서로 읽는 문서 |
