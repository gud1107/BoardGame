...
| 25 | 신규게임 | **스플렌더(Splendor) 신규 게임**(엔진→UI→온라인 방→카탈로그 등록, 9번째 플레이 가능 게임) | (docs/history.md Phase 25) |
| 26 | 버그수정 | 누락된 게임 박스 이미지 5종 일괄 연결(`image/` → `public/games/`) | `e70b870` |
| 27 | 신규게임 | **오이 다섯 개(Five Cucumbers) 신규 게임**(엔진→하우스룰→UI→온라인 방→카탈로그 등록, 10번째 플레이 가능 게임) | (이번 세션) |
...
## Phase 27 — 오이 다섯 개(Five Cucumbers) 신규 게임 (2026-08-09, 같은 날 열일곱 번째 세션)

**커밋**: (이번 세션 — `feat(five-cucumbers): add five cucumbers game engine, trick logic, cucumber count options, and ui`)

사용자가 `boardGameRuleBook/오이다섯개.md`(정확히는 `boardGameRule/오이다섯개.md`) 룰북을 근거로 오이 다섯 개(Five Cucumbers)를 10번째 플레이 가능 게임으로 구현해달라고 요청 — 엔진, 하우스 룰(탈락 기준 5개/6개 토글), UI, 온라인 멀티플레이어 동기화, 단위 테스트까지 전부 포함. 기존 9게임과 동일한 5파일 패턴(`engine.ts`/`<Game>Board.tsx`/`<Game>Game.tsx`/`RulebookModal.tsx`/`meta.ts`/`<Game>.test.ts`, 여기에 아이콘 전용 `CucumberIcon.tsx` 추가)으로 구현했다. 참고 템플릿은 "호스트가 방 생성 시 게임 모드를 고르고 그 선택이 `game-start` payload를 타고 전파되는" 구조가 가장 가까운 노땡스(`ChipVisibility`)를 그대로 따랐다.

### 1. 작업 지시(1~60 숫자 카드) vs 룰북 원문(1~15×4장) 불일치 — 룰북 채택
작업 지시문은 "1부터 60까지의 숫자 카드"라고 서술했지만, `boardGameRule/오이다섯개.md` §1 "구성물"은 명확히 "1부터 15까지의 숫자가 적힌 카드가 각 4장씩, 총 60장"이라고 밝히고 있다(오이 개수 5단계 구간표도 1~15 값 기준으로만 정의됨). 이 저장소의 반복된 작업 규칙(작업 지시와 룰북이 다른 사실을 말하면 룰북 원문을 채택하고 판단을 문서에 남길 것 — 센추리 Phase 12에서도 두 차례 있었던 상황)에 따라 **1~15×4장=60장** 구성을 그대로 구현했다. `engine.ts` 모듈 상단 doc에 이 판단 근거를 명시했다.

### 2. 하우스 룰(탈락 기준 5개/6개)은 룰북과 작업 지시가 실제로는 충돌하지 않음
룰북 §4는 "6개 이상이면 탈락, 하우스룰로 5개면 탈락"이라고 6을 본식·5를 변형으로 서술하지만, 작업 지시는 반대로 "기본값 5개"를 요청했다. 다만 이건 규칙 자체의 불일치가 아니라 **어느 값을 기본으로 노출할지**에 대한 UI 선택일 뿐이고, 5와 6 둘 다 룰북이 명시적으로 인정하는 값이라 실제 룰 충돌로 보지 않았다 — 노땡스의 `ChipVisibility`(비밀/공개 모드)와 같은 "호스트가 방 생성 화면에서 고르는 게임 모드" 패턴으로 `EliminationThreshold`를 만들고, 작업 지시대로 기본값만 5로 설정했다.

### 3. 엔진 설계 — 트릭 자동 정산 + 라운드 자동 재딜, 액션은 `playCard` 하나뿐 (`engine.ts`)
- **`EngineAction`이 `{ type: "playCard" }` 단 하나**다 — 트릭 완성(1~6번째: 승자 판정 후 바로 다음 트릭 리드로 전환) → 마지막 7번째 트릭 완성(오이 정산 + 탈락 판정) → 라운드 종료 시 다음 라운드 자동 재딜까지 전부 `playCard` 리듀서 안에서 순차적으로 처리된다. 스플렌더의 `finishTurn`→`afterTokensSettled` 2단계 게이트와 달리, 이 게임은 매 트릭·매 라운드 전환에 플레이어의 별도 확인 액션이 필요 없어(뭘 선택할지 묻는 지점이 없음) 게이트를 둘 필요가 없었다.
- **다음 라운드 재셔플은 새 네트워크 브로드캐스트 없이 결정론적으로 파생**된다 — `dealRound`가 `state.initialSeed + roundNumber * 104729`로 라운드마다 다른 시드를 계산해 모든 클라이언트가 동일하게 재셔플한다. 최초 1회만 `game-start`로 시드를 공유하는 다른 8게임과 달리, 이 게임은 라운드가 몇 번이고 반복될 수 있어 매 라운드 새 시드를 네트워크로 도는 것보다 공유 시드에서 파생시키는 편이 락스텝을 더 단순하게 유지한다(틀린 그림 찾기의 "좌표를 전송하지 않고 시드에서 파생" 원칙과 같은 종류의 판단).
- **탈락한 좌석은 손패를 비운 채 게임에 남아 회전 순서에서만 제외**된다(`nextActiveSeat`가 `eliminated` 좌석을 건너뜀) — 좌석 자체를 제거하지 않으므로 스코어보드에 계속 표시되고, `computeRankings`가 `eliminatedAtRound`(생존 못한 라운드 번호, null=끝까지 생존)로 표준 경쟁 순위를 매긴다. 같은 라운드에 2명 이상이 동시 탈락(마지막 트릭 동점 페널티로 둘 다 문턱을 넘는 경우 등)하거나, 마지막 남은 두 명이 같은 라운드에 동시 탈락(승자 없는 공동 최후)해도 별도 분기 없이 이 랭킹 스킴이 자동으로 공동 순위를 매긴다.
- **7번째 트릭의 동점 처리는 1~6번째 트릭과 의도적으로 다르다** — 1~6번째는 "나중에 낸 사람이 승리"(단일 승자, 다음 리드 결정용)지만, 7번째는 룰북이 명시한 대로 "동점자 전원이 각자" 페널티를 먹는다. `resolveLeadingTrickWinner`(단일 승자, `>=` 비교로 "나중 값이 동점을 덮어씀" 트릭 활용)와 7번째 트릭 전용 인라인 로직(`filter`로 동점자 전원 수집)을 분리해 구현했다.
- **1번 카드 배수는 승자의 카드가 아니라 트릭 전체에 나온 1번 카드 개수**로 계산한다(`onesCount = trickPlays.filter(v===1).length`, `multiplier = 2 ** onesCount`) — 룰북 예시(승자가 14를 냈는데 다른 사람이 1을 내면 배수 적용)와 정확히 일치.

### 4. 카드 UI — 오이 위험도에 따른 6단계 색상 그라디언트 (`FiveCucumbersBoard.tsx`, `CucumberIcon.tsx`)
카드 숫자 자체보다 "이 카드로 마지막 트릭을 이기면 오이를 몇 개 먹는가"가 실질적인 위험도이므로, 카드 배경을 숫자 구간이 아니라 `cucumberCount(value)`(0~5) 기준 6단계로 색을 입혔다(0=안전한 초록 → 5=위험한 진홍). 오이 개수는 카드 상단/하단에 순수 인라인 SVG 오이 아이콘(`CucumberIcon.tsx`, 이 프로젝트의 `GemToken.tsx`/`ResourceIcon.tsx`/`PerudoFaceIcon.tsx`와 같은 관례 — 외부 이미지 자산 없음)을 개수만큼 나열해 시각화했다. 손패에서 낼 수 있는 카드(현재 트릭 최댓값 이상이거나 손패 내 최저 카드)는 `legalCardIds`(엔진의 파생 predicate, 검증과 UI 하이라이트가 같은 함수 공유) 결과로 호박색 링+글로우를 씌우고, 낼 수 없는 카드는 반투명 처리 후 클릭을 막았다.

### 5. 트릭/라운드 결과 배너 — 상태 diff 기반 3.6초 토스트
`state.lastTrickResult`/`state.lastRoundSummary` 참조가 바뀌는 순간을 렌더 중 비교해(노땡스/센추리의 "prop이 바뀌면 즉시 감지" 패턴과 동일, effect 안에서 setState하지 않음) 트릭 승자 안내(1~6번째) 또는 오이 정산+탈락 안내(7번째, 1번 카드 배수 문구 포함)를 3.6초짜리 배너로 띄운다. 두 상태가 항상 같은 리듀서 호출에서 함께 갱신되므로 최종 트릭에서는 라운드 요약 배너가 우선 표시된다.

### 6. 검증
`npx tsc --noEmit`/`npm run lint`(경고 0)/`npx vitest run`(**372개 전부 통과**, 신규 28개 — 덱 구성, 트릭 제출 유효성(최댓값 이상 vs 손패 최저), 1~6번째 트릭 승자/다음 리드 판정, 7번째 트릭 오이 정산(1번 카드 배수, 동점자 전원 페널티), 하우스 룰 5개/6개 탈락 판정, 생존 순위 산정까지 요청받은 4개 영역 전부 커버) 전부 그린. `npm run dev` + `npx playwright screenshot`(CLI)로 고정 state를 주입한 임시 라우트(`/tmp-five-cucumbers-preview`, 검증 후 삭제)를 렌더링해 손패 유효/무효 하이라이트, 탈락 플레이어 배지, 마지막 트릭 경고 배너, 동점 순위(2,2,4) 결과 테이블까지 화면이 깨지지 않는 것을 확인했다 — 다만 이 프로젝트의 반복된 사각지대(§2)와 동일하게, 실제 Supabase 온라인 방 2대 이상 기기 동기화(특히 라운드가 여러 번 반복되며 탈락자가 발생하는 흐름)는 이번 세션에서도 검증하지 못했다.

---

## Phase 28 — 문서 재정리 + 모듈형 아키텍처 표준화 + 리소스 정리 (2026-08-09, 같은 날 열여덟 번째 세션)

**커밋**: `docs: refactor project documentation structure and establish scalable game plugin architecture`

사용자가 "보드게임이 무한히 확장되는 모듈형 아키텍처" 관점에서 `HANDOFF.md`/`README.md`/`boardGameRule/`/전체 프로젝트 구조를 재정리하고 중복을 완전히 제거해달라고 요청 — 아키텍처 표준 수립, 문서 슬림화, 룰북 정규화, 확인된 리소스 낭비 제거, 검증 후 커밋·푸시·배포까지 전부 포함.

### 1. 표준 구조 범위 결정 — 기존 검증된 패턴을 문서화(리팩터 없음)
사용자가 제시한 표준(`engine.ts`/`types.ts`/`components/`/`index.ts`)이 실제 10개 게임이 이미 쓰는 검증된 구조(`engine.ts`/`<Game>Board.tsx`/`<Game>Game.tsx`/`RulebookModal.tsx`/`meta.ts`/`<Game>.test.ts`, 플랫 파일, Vitest 전량 통과)와 달라, 진행 전 사용자에게 확인했다 — **기존 구조를 표준으로 문서화하는 쪽을 선택**(10개 게임 전체를 새 레이아웃으로 옮기는 건 기능 변화 없이 회귀 위험만 키우는 대규모 작업이라 판단). `meta.ts`(어디서도 import되지 않는 미사용 상수 1개짜리 파일, 전수 확인) 처리 방향도 함께 물어 **삭제**로 결정했다.

### 2. `ARCHITECTURE.md` 신설 — 표준 계약과 설계 근거 문서의 역할 분리
루트에 `ARCHITECTURE.md`를 새로 만들어 "신규 게임이 지켜야 할 규격"(엔진 순수성/불변성/결정론 3계약, 표준 모듈 레이아웃, 등록 4단계, 재사용해야 할 공용 유틸, 테스트 체크리스트)을 단일 출처로 명문화했다. 기존 `docs/architecture.md`는 "왜 이렇게 설계했는가"를 다루는 심화 설계 노트로 역할을 좁히고, 두 문서 맨 위에 서로를 가리키는 교차 링크를 추가해 앞으로 두 문서가 같은 내용을 독립적으로 다시 쓰지 않도록 했다.

### 3. 루트 문서 DRY 정리 — 실제 확인된 중복 3건 제거
- `README.md`의 게임별 개별 산문 섹션(7종 분량 약 90줄)이 이미 3종(스플렌더/오이다섯개/틀린그림찾기) 누락된 채 낡아 있었다 — 카탈로그 표 1개로 교체(HANDOFF.md와 내용을 공유하되 각자 갱신 부담이 적은 표 형식으로).
- `README.md`의 "설계 결정 및 이슈 기록" 섹션(Dexie.js/BettingContext 판단 등)이 `docs/architecture.md` §1.2/§1.3과 완전히 동일한 내용을 독립적으로 유지하고 있었다 — 전체 삭제 후 링크로 대체.
- `README.md`/`HANDOFF.md` 양쪽이 거의 같은 디렉토리 트리를 독립적으로 유지하며 이미 서로 어긋나 있었다(README가 낡은 버전) — `ARCHITECTURE.md`를 표준 레이아웃의 단일 출처로 두고 양쪽 다 축약 버전 + 링크로 정리.

### 4. `boardGameRule/` 정규화 — 시도했으나 동시 편집 충돌로 되돌림
모든 룰북을 `개요/구성품/핵심 룰/하우스 룰` 4단 헤더 구조로 맞추고, 프롬프트 잔여물(noThanks.md 상단의 메타 문장 등)을 제거하고, 파일명을 게임 ID와 1:1 매칭되는 kebab-case로 통일하는 작업(`스플랜더.md`→`splendor.md`, `오이다섯개.md`→`five-cucumbers.md`, `Grid Poker.md`→`grid-poker.md`, `Avalon.md`→`avalon.md`, `Century.md`→`century.md`, `Perudo.md`→`perudo.md`, `noThanks.md`→`no-thanks.md`, `spotTheDifference.md`→`spot-difference.md`, 미구현 게임 `포세일.md`→`for-sale.md`)을 실제로 전부 작성해 진행하던 중, **다른 세션(또는 프로세스)이 같은 `boardGameRule/` 폴더를 동시에 한글 파일명 규칙으로 재구성 중**임을 발견했다 — 빈 파일 `달무티.md`가 세션 도중(16:46) 생성되는 것으로 처음 감지했고, 이어서 `그리드포커.md`/`노땡스.md`/`뱅.md`/`센추리.md`/`아발론.md`/`틀린그림찾기.md`/`페루도.md`(기존 5개 게임을 한글 파일명으로 재구성한 것으로 보임)와 `라스베가스.md`/`러브레터.md`(완전히 새로운 게임 룰북 추가)가 실시간으로 나타났다. 이 저장소가 과거에도 겪은 "여러 세션이 같은 워킹 디렉터리를 동시에 쓰면 편집이 서로 덮어써진다"는 정확히 그 패턴([HANDOFF.md 작업 규칙](../HANDOFF.md), Phase 21 §0 참고)이라고 판단해, 사용자에게 확인 후 **`boardGameRule/`에 손댄 모든 변경을 마지막 커밋 상태로 완전히 되돌리고 이번 세션에서는 더 이상 건드리지 않기로 결정**했다(원본이 항상 마지막 커밋에 그대로 있었기 때문에 데이터 손실은 없었음). 되돌리는 과정에서 Windows(NTFS, 대소문자 구분 없음)에서 `avalon.md`를 새로 쓰면 이미 있던 `Avalon.md`와 물리적으로 같은 파일이 되어 버린다는 걸 두 번 겪었다 — 그 상태에서 `git rm`/`rm -f`를 실행하면 방금 쓴 새 내용째로 파일이 통째로 사라진다(내용 손실, 매번 즉시 재작성해 복구). **교훈**: 이 프로젝트처럼 파일명 대소문자만 다른 리네임(`Century.md`→`century.md` 등)을 할 때는 절대 "새 이름으로 쓰기 → 옛 이름 삭제" 순서로 하지 말고, 항상 완전히 다른 임시 파일명을 거쳐(`git rm` 먼저 또는 임시 경로에 쓴 뒤 이동) 두 파일이 동시에 같은 물리 경로를 가리키는 순간을 만들지 말 것. 표준(파일명 kebab-case, 4단 템플릿) 자체는 `ARCHITECTURE.md` §5에 "목표"로 문서화해뒀고, 다른 세션의 작업이 끝난 뒤 실제 적용은 다음 세션 과제로 남겼다(`HANDOFF.md` §3 1번).

### 5. 확인된 죽은 코드/에셋 제거 (전수 확인 후에만 삭제)
- 루트 `image/`(untracked) 삭제 — Phase 26에서 이미 전량 `public/games/`로 이관 완료된 원본 소스였고, 코드 전체 grep으로 미참조 확인.
- `src/games/*/meta.ts` 10개 삭제 — 각 파일의 export된 ID 상수를 전부 grep했으나 자기 자신 외 어디서도 import되지 않음을 확인.
- `seededRng`/`shuffle` 함수가 10개 `engine.ts`에 바이트 단위로 동일하게 복붙되어 있던 것을 `src/lib/rng.ts`로 추출 — 순수 함수 이동이라 동작 변화 없음(Vitest 전량 재통과로 검증). "게임 간 코드가 얽히지 않아야 한다"는 원칙과 "결정론적 유틸을 게임마다 재발명하지 않는다"는 원칙이 충돌하지 않는 지점(공용 유틸은 `src/lib/`에만 두고 게임 폴더끼리는 서로 참조하지 않음)임을 `ARCHITECTURE.md` §4에 명시.
- 점검했지만 정리할 게 없었던 항목: `package.json` 의존성 전수 확인(미사용 없음 — Three.js 계열은 페루도 3D 주사위에 실제로 쓰이고 있음), `public/games/*` 이미지 전수 확인(전부 `registry.ts`에서 참조됨, `catan.jpg`도 준비중 카드에 쓰임), `vite.config.ts`(이 프로젝트는 Next.js라 애초에 존재하지 않음).

### 6. 검증 및 배포
`npx tsc --noEmit`/`npm run lint`/`npx vitest run` 전부 재검증(rng 추출이 동작 보존 리팩터이므로 테스트 개수·결과 불변 확인) 후 한 커밋으로 푸시, `npx vercel deploy --prod`로 배포했다. 실제 결과는 이번 세션 커밋 로그와 `HANDOFF.md` 최신 스냅샷 참고.

## Phase 29+ — `HANDOFF.md` 대량 이관 아카이브 (2026-08-14 ~ 2026-09-12, 문서 분리 세션 2026-09-19)

> **이 섹션은 개별 Phase로 재정리하지 않고, `HANDOFF.md`에 쌓여 있던 원문을 그대로 옮겨왔다.** 2026-08-09 Phase 28 이후 이 문서(`history.md`)로의 이관이 약 40일간 누락되면서 `HANDOFF.md`가 9,200줄까지 불어났던 것을 2026-09-19 문서 정리 세션에서 발견해 한 번에 옮겼다(자세한 배경은 `HANDOFF.md`의 "문서 분리 이력" 메모 참고). 아래 항목들은 **원래 `HANDOFF.md`에 있던 순서(최신 날짜가 위) 그대로**이며 — 이 문서의 나머지 Phase 1~28(오래된 게 위, 최신이 아래)과 정렬 방향이 반대이니 날짜를 보고 읽을 것. 문구·판단 근거는 원문 그대로이고, 헤더만 `###`(원문 그대로)로 유지해 `## Phase N` 형식으로는 재번호를 매기지 않았다.
