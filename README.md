...
- **클라우드 DB (선택)**: [Supabase](https://supabase.com/) — 기기 간 플레이어 식별 매칭 + 내기 기록 백업용. 설정하지 않아도 앱의 나머지 기능은 정상 동작합니다. 단, **하나미코지의 온라인 대전은 Supabase Realtime이 반드시 필요합니다** (두 기기를 실시간으로 이어줄 다른 서버가 없기 때문) — 자세한 내용은 아래 "온라인 대전" 섹션 참고.
...
3. Supabase 기능(하나미코지 온라인 대전, 기기 간 식별 매칭, 내기 기록 클라우드 백업)을 쓰려면 Environment Variables에 `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`를 추가합니다. 비워두면 그 기능들만 "설정 필요" 화면으로 자동 비활성화되고 나머지(대시보드, 내기 정산 등)는 정상 동작합니다.
