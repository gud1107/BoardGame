...
export default function HistoryPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
      <h1 className="mb-1 text-2xl font-bold text-white light:text-slate-900">내기 기록</h1>
      ...
      {records === null ? (
        ...
        <div className="flex flex-col gap-4">
          {records.map((r) => (
            <div key={r.id} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 light:border-slate-200 light:bg-white light:shadow-sm">
              <div className="mb-3 flex items-center justify-between">
                <span className="text-sm font-semibold text-white light:text-slate-900">{r.date}</span>
                <span className="text-xs text-white/40 light:text-slate-400">{r.roundCount}게임 진행</span>
              </div>
