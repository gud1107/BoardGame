...
function RelicReferenceSidebar({ ownedRelicIds }: { ownedRelicIds: Set<string> }) {
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-3">
      <h3 className="text-xs font-semibold text-white/60">🏺 유물 점수표</h3>
      <div className="flex flex-col gap-2">
        {COUNTRIES.map((country) => (
          <div key={country} className="rounded-lg border border-white/5 bg-black/20 p-2">
            ...
            <div className="flex flex-col gap-0.5">
              {FORMATS.flatMap((format) => RELIC_DEFS.filter((r) => r.country === country && r.format === format)).map((r) => {
                ...
                return (
                  <div key={r.id} className={`flex items-center gap-1 text-[11px] ${owned ? "text-emerald-300" : "text-white/50"}`}>
                    ...
                    <span className="shrink-0 text-white/30">{r.baseScore}점</span>
                  </div>
                );
              })}
            </div>
  ...
  return (
    <div className="flex flex-col gap-4 lg:flex-row">
      ...
      <div className="order-1 flex flex-1 flex-col gap-4 lg:order-2">
        <div className="rounded-3xl border border-black/50 bg-gradient-to-b from-[#1a1023] via-[#120a17] to-[#0a0610] p-4 shadow-[0_0_60px_-20px_rgba(0,0,0,0.9)]">
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-sm font-bold text-white/80">🏛️ 위대한 유산 — {state.mode === "4p" ? "4인" : "8인"} 경매</h2>
            <span className="text-xs text-white/40">남은 매물 {state.deck.length + (auction ? 1 : 0)}장</span>
          </div>

          {auction && (
            <div className="mb-4 flex flex-col gap-2 rounded-2xl border border-white/10 bg-white/[0.04] p-3">
              <div className="flex items-center justify-between">
                ...
                <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${auction.kind === "reverse" ? "bg-rose-400/20 text-rose-200" : "bg-emerald-400/20 text-emerald-200"}`}>
                  {auction.kind === "reverse" ? "역경매 — 먼저 포기하면 낙찰" : "일반 경매"}
                </span>
              </div>
              <div className="text-xs text-white/50">
                현재 최고 입찰 <b className="text-amber-200">{auction.highestBid}코인</b>
                {auction.highestBidder !== null && ` (${names[auction.highestBidder] ?? "상대"})`} · 차례:{" "}
                <b className="text-white/80">{names[auction.activeSeat] ?? "상대"}</b>
              </div>
            </div>
          )}
          ...
          {state.phase === "gameOver" && rankings && (
            <div className="flex flex-col gap-3 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-4 text-center">
              <p className="text-lg font-bold text-white">🏆 게임 종료!</p>
              <div className="flex flex-col gap-1 text-sm text-white/80">
                {rankings.map((r) => (
                  <div key={r.seat} className="flex items-center justify-between rounded-lg bg-black/20 px-3 py-1.5">
                    <span>
