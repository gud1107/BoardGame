  ...
  return (
    <div
      className={`flex flex-col gap-2 rounded-2xl border p-3 transition ${
        isActive
          ? "border-amber-400/60 bg-amber-400/10 shadow-[0_0_20px_-6px_rgba(251,191,36,0.5)]"
          : hasPassed
    ...
    >
      <div className="flex items-center gap-2">
        <Avatar size={22} />
        <span className={`truncate text-sm font-semibold ${isMe ? "text-emerald-300" : "text-white"}`}>
          {name}
          {isMe && " (나)"}
        </span>
        {!isConnected && <span className="text-xs text-white/30">⏳</span>}
        {isActive && <span className="ml-auto rounded-full bg-amber-400/20 px-2 py-0.5 text-[10px] font-semibold text-amber-200">차례</span>}
        {hasPassed && <span className="ml-auto text-[10px] text-white/40">포기함</span>}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-white/70">
        ...
        <span className="font-semibold text-white">점수 {score.total}</span>
      </div>

      {(countries.length > 0 || formats.length > 0) && (
        <div className="flex flex-wrap gap-1">
          {countries.map((c) => (
            <span key={c} className="rounded-full bg-emerald-400/15 px-2 py-0.5 text-[10px] text-emerald-200">
              {COUNTRY_EMOJI[c] ?? "🏳️"} {c} 컬렉션 +3
            </span>
          ))}
          {formats.map((f) => (
            <span key={f} className="rounded-full bg-sky-400/15 px-2 py-0.5 text-[10px] text-sky-200">
