...
export default function MalDalliJaBoard({
  ...
  // 도입했던 안전장치를 2026-09-09 세션에서 게임오버 국면 한정 → 모든 이동으로
  // 확장(모바일에서 하얀 말이 이동 도중 사라져 안 보인다는 신고 조사 — 원인은
  // 이 게이트가 gameOver 국면에서만 발동했던 것). `AnimatedHorse`의 rAF 루프가
  // 매 프레임 정상 진행되면 `onDone`이 항상 불려 `handleAnimDone`이 그
  // 애니메이션을 `animations`에서 곧바로 제거하므로, 아래 타이머는 평소엔 절대
  // 발동하지 않는다(가장 긴 이동인 10칸 슬라이드도 1,300ms 안에 끝남). 하지만
  // 게임오버 국면이 아닌 **평범한 이동 도중에도** 모바일 브라우저의 탭
  // 백그라운드/화면 잠금으로 인한 rAF 스로틀링·정지, 네트워크 재동기화 등으로
  // `onDone`이 끝내 안 불리는 예외적 상황이 있을 수 있다 — 그러면
  // `state.positions`는 이미 새 칸을 가리키는데도 그 말이 `animatingKeys`에
  // 영구히 남아 정적 그리드 레이어(아래 렌더 루프의
  // `occupant && !animatingKeys.has(...)`)에서 계속 숨김 처리돼, 클릭/이동은
  // 정상인데 화면에서만 사라진 것처럼 보이는 상태가 게임이 끝날 때까지 영구
  // 고착된다("말이 사라짐" 신고와 정확히 일치). 그래서 게임 국면과 무관하게,
  // 애니메이션 하나하나마다 독립적으로 안전장치를 건다.
  const animationIds = animations.map((a) => a.id).join(",");
  useEffect(() => {
    if (animations.length === 0) return;
    // `animations` 배열이 바뀔 때마다(새 이동 추가/기존 이동 정상 종료 등) 이
    // effect가 다시 돌면서 그 시점에 남아있는 모든 애니메이션에 새 타이머를
    // 건다 — 이미 정상 종료된 항목은 `handleAnimDone`이 먼저 배열에서
        ...
        >
          ...
          {occupant && !animatingKeys.has(`${occupant.seat}-${occupant.horseIndex}`) && (
            ...
            >
              ...
              />
              {/* 2026-09-09: 링 색상(rose/cyan)만으로는 순간적인 하이라이트
                  전환(예: 상단 HUD의 "누구 차례" 강조 스타일 토글)이 마치 말
                  주인이 바뀐 것처럼 오인될 수 있어, 색상과 무관하게 고정
                  텍스트로 읽을 수 있는 식별 태그를 말 이미지 위에 항상 표시 —
        ...
        <div className="flex items-center gap-2">
          {(["p1", "p2"] as const).map((seat) => (
            ...
            >
              ...
              {/* 2026-09-09: 턴 강조 스타일(위 삼항연산자)이 켜졌다 꺼졌다 하는
