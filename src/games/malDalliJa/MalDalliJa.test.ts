...
describe("knight movement (§3 이동 방식 2)", () => {
  it("lands on any of the 8 L-shaped offsets when empty (starting square doesn't touch the oasis)", () => {
  ...
  it("can jump over an obstacle sitting between origin and landing square", () => {
    ...
    // knight moves don't check the path for *obstacles*, only the landing
    // square (the oasis-elbow house rule below is a separate, new check).
...
// ---------------------------------------------------------------------------
// New 2026-08-13/14 house rule — NOT in the rulebook (§3/§4 have no oasis
// carve-out for knight moves; §6 even recommends knight moves near the
// oasis). Requested and confirmed via AskUserQuestion despite that. See
// engine.ts's module doc for the full context.
// ---------------------------------------------------------------------------

describe("[하우스 룰] 오아시스 L자 이동 제약 (2026-08-13/14, 룰북에 없는 신규 규칙)", () => {
  it("a knight move that would land exactly on the oasis is not offered as a legal move", () => {
    const state = forceState({ positions: { p1: { row: 3, col: 4 }, p2: { row: 10, col: 10 } } });
    const moves = getLegalMoves(state).filter((m) => m.moveKind === "knight");
    expect(moves.map((m) => m.to)).not.toContainEqual(OASIS);
  });

  it("attempting to apply that blocked knight-onto-oasis move is a no-op", () => {
    const state = forceState({ positions: { p1: { row: 3, col: 4 }, p2: { row: 10, col: 10 } } });
    const next = applyAction(state, { type: "move", moveKind: "knight", dr: 2, dc: 1 });
    expect(next).toEqual(state); // still not moved, still "playing" — no win via knight anymore
  });

  it("a knight move is also blocked when the oasis is the 'elbow' of its L-shaped path, even though the landing square isn't the oasis (row-first decomposition)", () => {
    // From (3,5): offset (2,1) lands on (5,6) — not the oasis — but the
    // "move 2 rows, then turn 1 column" reading of that L bends around
    // (5,5), the oasis.
    const state = forceState({ positions: { p1: { row: 3, col: 5 }, p2: { row: 10, col: 10 } } });
    const moves = getLegalMoves(state).filter((m) => m.moveKind === "knight");
    expect(moves.map((m) => m.to)).not.toContainEqual({ row: 5, col: 6 });
    expect(moves.map((m) => m.to)).not.toContainEqual({ row: 5, col: 4 }); // offset (2,-1), same elbow
  });

  it("a knight move is also blocked via the other elbow decomposition (column-first)", () => {
    // From (5,3): offset (1,2) lands on (6,5) — not the oasis — but the
    // "move 2 columns, then turn 1 row" reading of that L bends around
    // (5,5), the oasis.
    const state = forceState({ positions: { p1: { row: 5, col: 3 }, p2: { row: 10, col: 10 } } });
    const moves = getLegalMoves(state).filter((m) => m.moveKind === "knight");
    expect(moves.map((m) => m.to)).not.toContainEqual({ row: 6, col: 5 });
    expect(moves.map((m) => m.to)).not.toContainEqual({ row: 4, col: 5 }); // offset (-1,2), same elbow
  });

  it("slide moves are completely unaffected by the oasis knight restriction — sliding onto the oasis still wins", () => {
    const state = forceState({ positions: { p1: { row: 3, col: 3 }, p2: { row: 6, col: 6 } } });
    const next = applyAction(state, { type: "move", moveKind: "slide", dr: 1, dc: 1 });
    expect(next.positions.p1).toEqual(OASIS);
    expect(next.winner).toBe("p1");
  });

  it("away from the oasis, knight (L자) moves are completely unrestricted — the general (노란색) zone allows all 8 offsets", () => {
    const state = forceState({ positions: { p1: { row: 8, col: 8 }, p2: { row: 0, col: 0 } } });
    const moves = getLegalMoves(state).filter((m) => m.moveKind === "knight");
    // (8,8) is an interior cell far from both the oasis (5,5) and the board
    // edge — all 8 offsets stay in-bounds, and none of their landing squares
    // or elbow cells come anywhere near the oasis, so all 8 are legal.
...
describe("chooseBotAction (AI bot support, Level 1–10)", () => {
  ...
  it("never proposes a knight move that the new oasis house rule blocks", () => {
    const state = forceState({ positions: { p1: { row: 3, col: 5 }, p2: { row: 10, col: 10 } } });
    for (let level = 1; level <= 10; level++) {
  ...
  it("Level 1 (forced onto its mistake path) slides away from the oasis, while Level 10 spots the immediate win", () => {
    ...
    expect(level1Action).toEqual({ type: "move", moveKind: "slide", dr: -1, dc: -1 });

    const level10Action = chooseBotAction(state, "p1", 10, () => 0);
    expect(level10Action).toEqual({ type: "move", moveKind: "slide", dr: 1, dc: 1 });
  });
});
