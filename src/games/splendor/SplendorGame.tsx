 ...
 * importing engine.ts. "purchaseCard" is Splendor's most game-defining
 * action: the entire game is a race to acquire development cards' victory
 * points.
 */
function formatSplendorPurchaseLog(name: string, points: number): string {
  return points > 0 ? `${name}님이 승점 ${points}점짜리 카드를 구매했습니다` : `${name}님이 개발 카드를 구매했습니다`;
}

/**
 * Online-room multiplayer entry point — same lockstep pattern as every other
 * `<Game>Game.tsx` in this project (closely modeled on century/CenturyGame.tsx,
 * see its header doc for the full rationale): every connected client
 * independently computes the full `SplendorState` (every seat's tokens,
 * purchased cards, AND reserved cards) from a shared RNG seed plus replayed
 ...
 * the accepted trust trade-off, and engine.ts's module doc for how reserved
 * cards (secret by the physical rules) stay hidden at the UI layer only.
 */
...
function getStoredSeat(code: string): number | null {
  const v = window.localStorage.getItem(`splendor-seat-${code}`);
...
function storeSeat(code: string, seat: number) {
  window.localStorage.setItem(`splendor-seat-${code}`, String(seat));
}
...
export default function SplendorGame({ onComplete }: PlayableGameProps) {
  ...
  const gameStateRef = useRef<SplendorState | null>(null);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`splendor-room-${roomCode}`, {
      config: { broadcast: { self: true }, presence: { key: deviceId } },
    });
    channelRef.current = channel;
    startSentRef.current = false;

    const chatChannel = `room:splendor:${roomCode}`;
      ...
      if (action.type === "purchaseCard") {
        ...
        const card =
          action.source === "reserved"
            ...
            : prevState
              ? ([1, 2, 3] as const).flatMap((t) => prevState.markets[t]).find((c) => c?.id === action.cardId)
              : undefined;
        setChatMessages((prev) => [
          ...prev,
          {
            ...
            deviceId: "system",
            senderName: "시스템",
            body: formatSplendorPurchaseLog(namesRef.current[action.seat] ?? "상대", card?.points ?? 0),
            type: "SYSTEM",
  ...
  const sendChatMessage = useCallback(
    (rawBody: string): SendResult => {
      ...
      const message: ChatMessage = {
        id: uuid(),
        channel: `room:splendor:${roomCode}`,
        deviceId,
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      if (!state) return;
      const actor = splendorCurrentActor(state);
    ...
    return withGuard(
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-8 text-center light:border-amber-400/40 light:bg-amber-50 light:shadow-sm">
        ...
        <p className="max-w-sm text-sm text-amber-100/80 light:text-amber-800">
          스플렌더는 실시간 온라인 대전 전용이라 Supabase 설정이 필요합니다.
  ...
  if (phase === "room-full") {
    return withGuard(
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-rose-400/30 bg-rose-400/10 p-8 text-center light:border-rose-300 light:bg-rose-50 light:shadow-sm">
        ...
        <button onClick={handleLeave} className="mt-2 rounded-full bg-violet-600 px-5 py-2 text-sm font-semibold text-white hover:bg-violet-500">
          처음으로
        </button>
      </div>
    );
        ...
        actions={
          <div className="mt-2 flex w-full max-w-xs flex-col gap-2">
            ...
            <button
              ...
              className="w-full rounded-xl border border-white/15 py-3 text-sm font-semibold text-white/80 transition hover:border-white/30 light:border-slate-200 light:text-slate-700 light:hover:border-slate-300"
            >
              🔑 초대 코드로 참여
            </button>
          </div>
    ...
    return withGuard(
      <div className="flex flex-col gap-4 rounded-2xl border border-white/10 bg-white/[0.03] p-6 light:border-slate-200 light:bg-white light:shadow-sm">
        ...
        {intent === "join" && (
          <label className="flex flex-col gap-1.5 text-sm text-white/70 light:text-slate-600">
            ...
            <input
              ...
              placeholder="0000"
              inputMode="numeric"
              className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-center text-lg font-semibold tracking-[0.3em] text-white placeholder:text-white/20 focus:border-violet-400 focus:outline-none light:border-slate-200 light:bg-slate-50 light:text-slate-900 light:placeholder:text-slate-300"
            />
          </label>
        )}
        {intent === "create" && (
          <label className="flex flex-col gap-1.5 text-sm text-white/70 light:text-slate-600">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="h-8 w-8 rounded-full border border-white/15 text-white/80 hover:border-white/30 light:border-slate-200 light:text-slate-700 light:hover:border-slate-300"
              >
                −
              </button>
        ...
        <div className="flex gap-2">
          <button onClick={() => setPhase("choose")} className="flex-1 rounded-xl border border-white/15 py-2.5 text-sm text-white/70 hover:border-white/30 light:border-slate-200 light:text-slate-700 light:hover:border-slate-300">
            뒤로
          </button>
          <button onClick={enterRoom} className="flex-1 rounded-xl bg-violet-600 py-2.5 text-sm font-semibold text-white hover:bg-violet-500">
      ...
      <div className="flex flex-col items-center gap-5 rounded-2xl border border-white/10 bg-white/[0.03] p-8 text-center light:border-slate-200 light:bg-white light:shadow-sm">
        {phase === "connecting" ? (
          ...
          <>
            ...
            <button onClick={() => navigator.clipboard?.writeText(shareUrl)} className="rounded-full border border-white/15 px-4 py-2 text-xs text-white/70 hover:border-white/30 light:border-slate-200 light:text-slate-700 light:hover:border-slate-300">
              🔗 초대 링크 복사
            </button>
  ...
  if (phase === "playing" && gameState && mySeat !== null) {
    ...
    return withGuard(
      ...
      <SplendorBoard state={gameState} viewerSeat={mySeat} names={names} connectedSeats={connectedSeats} onAction={handleAction} onGameEnd={handleGameEnd} />
  ...
  if (phase === "post-game" && finalResult) {
    return withGuard(
      ...
      <div className="flex flex-col items-center gap-5 rounded-2xl border border-white/10 bg-white/[0.03] p-8 text-center light:border-slate-200 light:bg-white light:shadow-sm">
        ...
        <div className="flex gap-2">
          <button onClick={handleLeave} className="rounded-xl border border-white/15 px-4 py-2.5 text-sm text-white/70 hover:border-white/30 light:border-slate-200 light:text-slate-700 light:hover:border-slate-300">
            나가기
          </button>
          <button onClick={handleRematch} className="rounded-xl bg-violet-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-violet-500">
            다시하기
          </button>
        </div>
      </div>
