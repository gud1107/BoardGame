 ...
 * `<Game>Game.tsx` in this project (docs/cloud-sync.md): every connected
 * client independently computes the full `RatATatCatState` (including every
 * seat's hidden cards) from a shared RNG seed plus replayed `EngineAction`s —
 * there is no server-authoritative engine. `chatEnabled`/`bettingRoomLinked`
 * are deliberately NOT wired here — neither is in the work order's "공통
 * 규격" list, same call as Lost Cities (see engine.ts docstring / HANDOFF.md).
 */
...
function getStoredSeat(code: string): number | null {
  const v = window.localStorage.getItem(`rat-a-tat-cat-seat-${code}`);
...
function storeSeat(code: string, seat: number) {
  window.localStorage.setItem(`rat-a-tat-cat-seat-${code}`, String(seat));
}
...
export default function RatATatCatGame({ onComplete }: PlayableGameProps) {
  ...
  const [gameState, setGameState] = useState<RatATatCatState | null>(null);
  ...
  const gameStateRef = useRef<RatATatCatState | null>(null);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`rat-a-tat-cat-room-${roomCode}`, {
    ...
    channel.on("broadcast", { event: "state-sync" }, ({ payload }) => {
      const state = payload?.state as RatATatCatState | undefined;
      if (!state) return;
  ...
  function withGuard(node: ReactNode) {
    return (
      <>
        ...
        {showRulebook && <RatATatCatRulebookModal onClose={() => setShowRulebook(false)} />}
      </>
    );
  }
    ...
    return withGuard(
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-8 text-center">
        ...
        <p className="max-w-sm text-sm text-amber-100/80">
          랫어탯캣은 실시간 온라인 대전 전용이라 Supabase 설정이 필요합니다.
  ...
  if (phase === "choose") {
    return withGuard(
      <RulebookGate
        gameId="rat-a-tat-cat"
        icon="🐱"
        title="랫어탯캣 온라인 대전"
        description={<p className="text-sm text-white/50">2~6명이 각자 기기로 접속해서 실시간으로 플레이해요.</p>}
        actions={
                ...
                return (
                  <div key={seat} className="flex items-center justify-between gap-3 text-sm text-white/70">
                    <span className="flex items-center gap-1.5">
                      {occ && <Avatar size={20} />}
                      {seat === mySeat ? "나" : `${seat + 1}번`}: {occ ? occ.name : isBot ? <BotSeatBadge label={botLabel(botIdx, botLevels[botIdx])} /> : <span className="text-white/30">대기 중...</span>}
                    </span>
  ...
  if (phase === "playing" && gameState && mySeat !== null) {
    ...
    return withGuard(
      <>
        ...
        <RatATatCatBoard state={gameState} viewerSeat={mySeat} names={names} connectedSeats={connectedSeats} onAction={handleAction} onGameEnd={handleGameEnd} />
      </>
    );
  }
