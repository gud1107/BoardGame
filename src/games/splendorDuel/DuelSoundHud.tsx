...
/**
 * 🎵 lute BGM / 🔔 SFX toggles next to the rulebook button. Both write the
...
export default function DuelSoundHud() {
  ...
  function toggle(kind: "bgm" | "sfx") {
    ...
    // Preview so the player hears it's live (the lute starts on its own).
  ...
  return (
    <span className="flex shrink-0 items-center gap-1">
      <button type="button" onClick={() => toggle("bgm")} className={cls(bgmOn)} title={bgmOn ? "류트 BGM 끄기 (볼륨은 상단 설정)" : "류트 BGM 켜기"} aria-pressed={bgmOn}>
        <span className={bgmOn ? "" : "opacity-50 grayscale"}>🎵</span>
