...
import SplendorDuelBoard from "./SplendorDuelBoard";
import SplendorDuelRulebookModal from "./RulebookModal";
...
/**
 * Online-room multiplayer entry point for 스플렌더 대결 — a direct copy of
 * `lostCities/LostCitiesGame.tsx`'s 2-player lockstep wiring: the host
 * broadcasts a seed, both clients derive the identical board/deck
 * (`startGame`), and every move replays as an `EngineAction` through the same
 * pure reducer. Only the active seat ever sends a move — that includes the
 * follow-up choices (steal / copy / royal / discard), which the engine always
 * assigns to the active seat, so the single-writer guarantee holds.
 *
 * Same scope as Lost Cities: bot seats, vote-based bot takeover on
 * disconnect/idle, leave guard, background resync and the lobby
 * active-room listing are wired in; room chat and room-linked betting are not.
 */
...
function getStoredRole(code: string): Seat | null {
  const v = window.localStorage.getItem(`splendor-duel-role-${code}`);
  return v === "p1" || v === "p2" ? v : null;
}

function storeRole(code: string, role: Seat) {
  window.localStorage.setItem(`splendor-duel-role-${code}`, role);
}

/** Whose decision `useBotAutoplay` should drive right now. */
function sdCurrentActor(state: SplendorDuelState): Seat | null {
  // Includes the "resolving" phase — ability/royal/discard choices belong to the active seat too.
  return currentActor(state);
}

export default function SplendorDuelGame({ onComplete }: PlayableGameProps) {
  ...
  const [gameState, setGameState] = useState<SplendorDuelState | null>(null);
  ...
  const gameStateRef = useRef<SplendorDuelState | null>(null);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`splendor-duel-room-${roomCode}`, {
    ...
    channel.on("broadcast", { event: "state-sync" }, ({ payload }) => {
      const syncedState = payload?.state as SplendorDuelState | undefined;
  ...
  const chooseAction = useCallback((state: SplendorDuelState, actor: Seat): EngineAction | null => {
  ...
  useBotAutoplay<SplendorDuelState, EngineAction, Seat>({
    active: isHost && phase === "playing",
    state: gameState,
    currentActor: sdCurrentActor,
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      if (!state) return;
      const actor = sdCurrentActor(state);
  ...
  useEffect(() => {
    if (!gameState || gameState.phase !== "gameOver" || !gameState.winner || completedRef.current) return;
    completedRef.current = true;
    const winner = gameState.winner;
    const rankings = [
      { playerId: ids[winner], rank: 1 },
      { playerId: ids[otherSeat(winner)], rank: 2 },
    ];
  ...
  // best-effort, see useActiveRoomListing.ts. Splendor Duel is a fixed 2-seat
  ...
  function withGuard(node: ReactNode) {
    return (
      <>
        ...
        {showRulebook && <SplendorDuelRulebookModal onClose={() => setShowRulebook(false)} />}
      </>
    );
  }
    ...
    return withGuard(
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-8 text-center">
        ...
        <p className="max-w-sm text-sm text-amber-100/80">
          스플렌더 대결은 실시간 온라인 대전 전용이라 Supabase 설정이 필요합니다.{" "}
  ...
  if (phase === "room-full") {
    return withGuard(
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-rose-400/30 bg-rose-400/10 p-8 text-center">
        ...
        <button onClick={handleLeave} className="mt-2 rounded-full bg-amber-500 px-5 py-2 text-sm font-semibold text-white hover:bg-amber-400">
          처음으로
        </button>
      </div>,
    );
  ...
  if (phase === "choose") {
    return withGuard(
      <RulebookGate
        gameId="splendor-duel"
        containerClassName="border-white/10 bg-gradient-to-b from-[#1c1407] via-[#0c0a09] to-black light:border-slate-200 light:bg-none light:bg-white"
        icon="💎"
        title="스플렌더 대결 온라인 대전"
  ...
  if (phase === "enter-name") {
    return withGuard(
      <div className="flex flex-col gap-4 rounded-2xl border border-white/10 bg-gradient-to-b from-[#1c1407] via-[#0c0a09] to-black p-6">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={enterRoom} className="flex-1 rounded-xl bg-amber-500 py-2.5 text-sm font-semibold text-white hover:bg-amber-400">
  ...
  if (phase === "connecting" || phase === "waiting") {
    return withGuard(
      <div className="flex flex-col items-center gap-5 rounded-2xl border border-white/10 bg-gradient-to-b from-[#1c1407] via-[#0c0a09] to-black p-8 text-center">
        {phase === "connecting" ? (
