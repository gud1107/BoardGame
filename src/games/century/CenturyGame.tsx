...
import CenturyBoard from "./CenturyBoard";
 ...
 * already-resolved plain name + point value instead of importing engine.ts,
 * so the pure reducer stays untouched. e.g. "지수님이 12점 카드를 획득했습니다"
 * for a `claimPoint` action — the single most game-defining "scored a point
 * card" moment in Century: Spice Road.
 */
function formatCenturyClaimPointLog(name: string, points: number): string {
  return `${name}님이 ${points}점 카드를 획득했습니다`;
}

/** Whose decision `useBotAutoplay` should drive right now — the mandatory §6 discard temporarily hands control to `awaitingDiscardSeat` instead of `activeSeat`. */
function centuryCurrentActor(state: CenturyState): SeatIndex | null {
 ...
 * `<Game>Game.tsx` in this project (see perudo/PerudoGame.tsx, this file is
 * closely modeled on it): every connected client independently computes the
 * full `CenturyState` (every seat's hand + resources) from a shared RNG seed
 * plus replayed `EngineAction`s broadcast over Supabase Realtime — there is
 * no server-authoritative engine. See engine.ts and docs/architecture.md §2
 * for the accepted trust trade-off.
 */
...
function getStoredSeat(code: string): number | null {
  const v = window.localStorage.getItem(`century-seat-${code}`);
...
function storeSeat(code: string, seat: number) {
  window.localStorage.setItem(`century-seat-${code}`, String(seat));
}
...
export default function CenturyGame({ onComplete }: PlayableGameProps) {
  ...
  const [gameState, setGameState] = useState<CenturyState | null>(null);
  ...
  const gameStateRef = useRef<CenturyState | null>(null);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`century-room-${roomCode}`, {
      config: { broadcast: { self: true }, presence: { key: deviceId } },
    });
    channelRef.current = channel;
    startSentRef.current = false;

    const chatChannel = `room:century:${roomCode}`;
    ...
    channel.on("broadcast", { event: "game-action" }, ({ payload }) => {
      ...
      // action log anyway.
      if (action.type === "claimPoint") {
        const card = gameStateRef.current?.pointMarket[action.index];
        if (card) {
          setChatMessages((prev) => [
            ...prev,
            {
              ...
              deviceId: "system",
              senderName: "시스템",
              body: formatCenturyClaimPointLog(namesRef.current[action.seat] ?? "상대", card.points),
              type: "SYSTEM",
    ...
    // one-time `game-start` broadcast — see PerudoGame.tsx for the identical
    // reconnect flow this mirrors.
    ...
    channel.on("broadcast", { event: "state-sync" }, ({ payload }) => {
      const state = payload?.state as CenturyState | undefined;
      if (!state) return;
      const roster = (payload?.botSeats as SeatIndex[] | undefined) ?? [];
      const levels = (payload?.botLevels as BotLevel[] | undefined) ?? [];
  ...
  const chooseAction = useCallback((state: CenturyState, actor: SeatIndex): EngineAction | null => {
  ...
  useBotAutoplay<CenturyState, EngineAction, SeatIndex>({
    active: isHost && phase === "playing",
    state: gameState,
    currentActor: centuryCurrentActor,
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      if (!state) return;
      const actor = centuryCurrentActor(state);
    ...
    return withGuard(
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-8 text-center">
        ...
        <p className="max-w-sm text-sm text-amber-100/80">
          센추리는 실시간 온라인 대전 전용이라 Supabase 설정이 필요합니다.
