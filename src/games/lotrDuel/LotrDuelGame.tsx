...
/**
 * Online-room multiplayer entry point for 반지의 제왕: 가운데땅에서의 대결 —
 * the same 2-player Supabase lockstep wiring as `splendorDuel/SplendorDuelGame.tsx`
 * (itself a copy of Lost Cities'): the host broadcasts a seed + which seat
 ...
 * listing are wired in; room chat and room-linked betting are not.
 */
...
export default function LotrDuelGame({ onComplete }: PlayableGameProps) {
  ...
  // Host's side pick in the waiting room — resolved to a concrete seat when the game starts.
  const [hostFaction, setHostFaction] = useState<HostFaction>("RANDOM");
  ...
  useEffect(() => {
    ...
    channel.on("broadcast", { event: "game-start" }, ({ payload }) => {
      ...
      const fellowshipSeat = (payload?.fellowshipSeat as Seat | undefined) ?? "p1";
  ...
  useEffect(() => {
    ...
    if (hasP1 && hasP2) {
      ...
      const hostIsFellowship = hostFaction === "RANDOM" ? Math.random() < 0.5 : hostFaction === "FELLOWSHIP";
      channelRef.current?.send({
        type: "broadcast",
        event: "game-start",
        payload: {
          seed: Math.floor(Math.random() * 1_000_000_000),
          fellowshipSeat: hostIsFellowship ? "p1" : "p2",
          botRoles: botRolesRef.current,
          botLevels: botLevelsRef.current,
        },
      });
    }
  }, [occupants, botRoles, phase, isHost, hostFaction]);
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      if (!state) return;
      const actor = lotrCurrentActor(state);
  ...
  }, [phase, roomCode, lotrCurrentActor]);
  ...
  useEffect(() => {
    ...
    const winner = seatOf(gameState, gameState.winner);
    const rankings = [
  ...
  // Rematch swaps sides — the other seat plays the Fellowship this time.
  function handleRematch() {
    startSentRef.current = true;
    const prevFellowship = gameStateRef.current ? seatOf(gameStateRef.current, "FELLOWSHIP") : "p1";
    channelRef.current?.send({
      type: "broadcast",
      event: "game-start",
      payload: {
        seed: Math.floor(Math.random() * 1_000_000_000),
        fellowshipSeat: otherSeat(prevFellowship),
        botRoles: botRolesRef.current,
        botLevels: botLevelsRef.current,
      },
    });
  ...
  function withGuard(node: ReactNode) {
    return (
      <>
        ...
        {showRulebook && <LotrRulebookModal onClose={() => setShowRulebook(false)} />}
      </>
    );
  }
  ...
  if (phase === "enter-name") {
    return withGuard(
      <div className="flex flex-col gap-4 rounded-2xl border border-white/10 bg-gradient-to-b from-[#15121c] via-[#0b0a10] to-black p-6">
  ...
  if (phase === "connecting" || phase === "waiting") {
    return withGuard(
      <div className="flex flex-col items-center gap-5 rounded-2xl border border-white/10 bg-gradient-to-b from-[#15121c] via-[#0b0a10] to-black p-8 text-center">
        {phase === "connecting" ? (
          ...
          <>
            ...
            {isHost ? (
              <div className="flex flex-col items-center gap-1.5">
                ...
                <div className="flex gap-1.5">
                  {(
                    ...
                    <button
                      key={value}
                      onClick={() => setHostFaction(value)}
                      className={`rounded-full border px-3 py-1.5 text-xs font-semibold transition ${hostFaction === value ? "border-amber-400 bg-amber-500/20 text-amber-200" : "border-white/15 text-white/60 hover:border-white/30"}`}
                    >
                      {label}
                    </button>
                  ))}
              ...
              <p className="text-xs text-white/40">진영은 방장이 정합니다 (재대결 시 진영 교대).</p>
            )}
