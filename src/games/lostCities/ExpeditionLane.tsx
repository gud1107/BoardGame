...
/** Light-mode pastel counterparts for `LANE_THEME`'s dark `backdropFrom`/`backdropTo` gradients — kept local since the dark values encode a per-color "night wash" that has no single formula to derive a light tint from. */
const LANE_LIGHT_BACKDROP: Record<Color, string> = {
  yellow: "light:from-yellow-100 light:to-yellow-50",
  blue: "light:from-blue-100 light:to-blue-50",
  white: "light:from-slate-100 light:to-white",
  green: "light:from-green-100 light:to-green-50",
  red: "light:from-red-100 light:to-red-50",
};
...
export default function ExpeditionLane({ color, cards, highlight, dim, onClick, laneRef }: ExpeditionLaneProps) {
  ...
  return (
    <div
      ref={laneRef}
      className={`flex flex-1 flex-col items-center gap-0.5 rounded-lg border bg-gradient-to-b p-0.5 transition light:shadow-sm ${theme.backdropFrom} ${theme.backdropTo} ${LANE_LIGHT_BACKDROP[color]} ${
        highlight ? "lc-target-beam-pulse border-emerald-300 ring-2 ring-emerald-300/70" : `${theme.laneBorder} light:border-slate-300`
      } ${dim ? "opacity-60" : ""}`}
    >
      <div className="flex w-full items-center justify-between px-0.5">
        <span className="text-[9px] leading-none text-white/60 light:text-slate-600 sm:text-[10px]" title={theme.label}>
      ...
      <div className="flex min-h-[1.6rem] w-full flex-col items-center justify-center leading-none">
        {!started ? (
          <span className="text-[9px] text-white/25 light:text-slate-400">미시작</span>
        ) : (
          <>
            <span
              key={breakdown.total}
              className={`lc-score-pulse text-[11px] font-extrabold sm:text-xs ${breakdown.total >= 0 ? "text-emerald-300 light:text-emerald-600" : "text-rose-400 light:text-rose-600"}`}
            >
              {breakdown.total >= 0 ? `+${breakdown.total}` : breakdown.total}
            </span>
            <span className="text-[8px] text-rose-300/80 light:text-rose-600 sm:text-[9px]">시작비용 -20</span>
            {breakdown.bonus > 0 && (
      ...
      <button
        ...
        disabled={!onClick}
        onClick={onClick}
        className="flex min-h-[3.5rem] w-full flex-1 items-center gap-[-6px] overflow-x-auto rounded-md border border-dashed border-white/15 bg-black/20 p-1 light:border-slate-300 light:bg-white/50 sm:min-h-[4.5rem]"
      >
        {cards.length === 0 ? (
          <span className="mx-auto text-[10px] text-white/25 light:text-slate-400">비어있음</span>
        ) : (
