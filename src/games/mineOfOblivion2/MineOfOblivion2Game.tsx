...
import { applyAction, chooseBotAction, otherSeat, startGame, type EngineAction, type MineOfOblivion2State, type Seat } from "./engine";
import MineOfOblivion2Board from "./MineOfOblivion2Board";
import MineOfOblivion2RulebookModal from "./RulebookModal";
...
/**
 * Online-room multiplayer entry point for 망각의 지뢰 2 — same lockstep
 * pattern every 2-player game in this catalog uses (docs/cloud-sync.md),
 * cloned directly from 1편's `MineOfOblivionGame.tsx` (identical room/bot/
 * takeover machinery; the only real differences from 1편 live in `engine.ts`
 * and the two UI files). The host broadcasts a seed, both clients
 * independently derive the identical initial state (`startGame`), and every
 * action replays as an `EngineAction` broadcast through the same pure
 * reducer.
 *
 * `REVEAL_STEP` is the one shared-clock exception: the room host alone
 * dispatches `READY_NEXT_ROUND` once `REVEAL_SECONDS` has elapsed, and any
 * client's Board.tsx skip button can also fire it early.
 */
...
function getStoredRole(code: string): Seat | null {
  const v = window.localStorage.getItem(`mine-of-oblivion-2-role-${code}`);
  return v === "p1" || v === "p2" ? v : null;
}

function storeRole(code: string, role: Seat) {
  window.localStorage.setItem(`mine-of-oblivion-2-role-${code}`, role);
}

/** Whose decision `useBotAutoplay` should drive right now — same convention as 1편's `mooCurrentActor`. */
function moo2CurrentActor(state: MineOfOblivion2State): Seat | null {
...
export default function MineOfOblivion2Game({ onComplete }: PlayableGameProps) {
  ...
  const [gameState, setGameState] = useState<MineOfOblivion2State | null>(null);
  ...
  const gameStateRef = useRef<MineOfOblivion2State | null>(null);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`mine-of-oblivion-2-room-${roomCode}`, {
    ...
    channel.on("broadcast", { event: "state-sync" }, ({ payload }) => {
      const syncedState = payload?.state as MineOfOblivion2State | undefined;
  ...
  const chooseAction = useCallback((state: MineOfOblivion2State, actor: Seat): EngineAction | null => {
  ...
  useBotAutoplay<MineOfOblivion2State, EngineAction, Seat>({
    active: isHost && phase === "playing",
    state: gameState,
    currentActor: moo2CurrentActor,
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- keyed to this reveal episode itself, same pattern as 1편
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      if (!state) return;
      const actor = moo2CurrentActor(state);
  ...
  // best-effort, see useActiveRoomListing.ts. Mine of Oblivion 2 is a fixed
  ...
  function withGuard(node: ReactNode) {
    return (
      <>
        ...
        {showRulebook && <MineOfOblivion2RulebookModal onClose={() => setShowRulebook(false)} />}
      </>
    );
  }
        ...
        actions={
          <div className="mt-2 flex w-full max-w-xs flex-col gap-2">
            <button
              ...
              className="w-full rounded-xl bg-orange-500 py-3 text-sm font-semibold text-white transition hover:bg-orange-400"
            >
              🎲 방 만들기
            </button>
            <button
  ...
  if (phase === "playing" && gameState && myRole) {
    ...
    return withGuard(
      <>
        ...
        <MineOfOblivion2Board state={gameState} viewerSeat={myRole} names={names} opponentConnected={opponentConnected} onAction={handleAction} onLeave={handleLeave} onRematch={handleRematch} />
      </>,
    );
  }
