 ...
 * state to see others' roles/hands).
 */
...
function getStoredSeat(code: string): number | null {
  const v = window.localStorage.getItem(`bang-seat-${code}`);
...
function storeSeat(code: string, seat: number) {
  window.localStorage.setItem(`bang-seat-${code}`, String(seat));
}
...
export default function BangGame({ onComplete }: PlayableGameProps) {
  ...
  const gameStateRef = useRef<BangState | null>(null);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`bang-room-${roomCode}`, {
    ...
    // guaranteed to reflect earlier joiners yet, and claiming a seat from an
    // empty-but-stale snapshot is what let two near-simultaneous joiners both
    // claim seat 0 (a real bug caught by multi-tab testing, not a hypothetical).
  ...
  // instant (presence sync isn't guaranteed to reflect an earlier joiner by
  // the time a later one claims its seat — see the comment on `firstSync`
  // above). Rather than immediately failing the room, self-heal with a
  // deterministic tie-break: whichever device has the lexicographically
  // larger id gives up the seat and claims the next free one. Only after a
  // few failed attempts (or once the game has actually started, when
  // reseating no longer makes sense) do we fall back to "room-full", which
  // is now reserved for the genuine case — a stale seat from an unrelated
  // past session in this same room code.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      if (!state) return;
      const actor = bangCurrentActor(state);
  ...
  function handleGameEnd() {
    ...
    const rankings = gameState.turnOrder.map((seat) => {
      const p = gameState.players[seat];
      const isWinner =
        winner === "renegade"
          ? p.role === "renegade" && p.alive
          : winner === "outlaw"
            ? p.role === "outlaw"
            : p.role === "sheriff" || p.role === "deputy";
      return { playerId: ids[seat], rank: isWinner ? 1 : 2 };
    });
  ...
  if (phase === "supabase-missing") {
    return withGuard(
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-8 text-center light:border-amber-400/50 light:bg-amber-50">
        ...
        <p className="max-w-sm text-sm text-amber-100/80 light:text-amber-800">
          뱅!은 실시간 온라인 대전 전용이라 Supabase 설정이 필요합니다.
          <code className="mx-1 rounded bg-black/30 px-1.5 py-0.5 text-xs light:bg-white/80 light:text-slate-700">.env.local</code>
          에 <code className="rounded bg-black/30 px-1.5 py-0.5 text-xs light:bg-white/80 light:text-slate-700">NEXT_PUBLIC_SUPABASE_URL</code> /
          <code className="mx-1 rounded bg-black/30 px-1.5 py-0.5 text-xs light:bg-white/80 light:text-slate-700">NEXT_PUBLIC_SUPABASE_ANON_KEY</code>
          를 채워주세요 (README 참고).
        </p>
      </div>
    );
  ...
  if (phase === "choose") {
    return withGuard(
      <RulebookGate
        ...
        icon="🤠"
        title="뱅! 온라인 대전"
        description={<p className="text-sm text-white/50 light:text-slate-500">4~8명이 각자 기기로 접속해서 실시간으로 플레이해요.</p>}
        actions={
        ...
        {intent === "join" && (
          <label className="flex flex-col gap-1.5 text-sm text-white/70 light:text-slate-600">
            ...
            <input
              ...
              placeholder="0000"
              inputMode="numeric"
              className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-center text-lg font-semibold tracking-[0.3em] text-white placeholder:text-white/20 focus:border-amber-400 focus:outline-none light:border-slate-300 light:bg-white light:text-slate-900 light:placeholder:text-slate-300"
            />
          </label>
        )}
        {intent === "create" && (
          <label className="flex flex-col gap-1.5 text-sm text-white/70 light:text-slate-600">
            ...
            <div className="flex items-center gap-3">
              <button
                type="button"
                onClick={() => setTargetPlayerCount((n) => Math.max(4, n - 1))}
    ...
    return withGuard(
      ...
      <div className="flex flex-col items-center gap-5 rounded-2xl border border-white/10 bg-white/[0.03] p-8 text-center light:border-slate-200 light:bg-white/90 light:shadow-sm">
        ...
        <p className="text-white/80 light:text-slate-700">
          {finalResult.winner === "law" ? "보안관 팀" : finalResult.winner === "outlaw" ? "무법자 팀" : "배신자"} 승리로
          게임이 끝났어요.
        </p>
        <div className="flex gap-2">
          <button
