...
import { applyAction, chooseBotAction, otherSeat, startGame, type EngineAction, type MineOfOblivionState, type Seat } from "./engine";
import MineOfOblivionBoard from "./MineOfOblivionBoard";
import MineOfOblivionRulebookModal from "./RulebookModal";
...
/**
 * Online-room multiplayer entry point for 망각의 지뢰 — the same lockstep
 * pattern every 2-player game in this catalog uses (docs/cloud-sync.md),
 * modeled directly on `lostCities/LostCitiesGame.tsx`: the host broadcasts a
 * seed, both clients independently derive the identical initial state
 * (`startGame`), and every action replays as an `EngineAction` broadcast
 * through the same pure reducer. Only the active seat (or, during
 * `SETUP_MINE`, either not-yet-ready seat) ever sends a move-consuming
 * action — single-writer-per-seat guarantee.
 *
 * `REVEAL_STEP` is the one shared-clock exception (see engine.ts's doc): the
 * room host alone dispatches `READY_NEXT_ROUND` once `REVEAL_SECONDS` has
 * elapsed (mirrors grid-poker's `round-result`→`advance-round-result`), and
 * any client's Board.tsx skip button can also fire it early — same
 * "host timer + shared skip" split as every other dramatic-reveal game here.
 */
...
function getStoredRole(code: string): Seat | null {
  const v = window.localStorage.getItem(`mine-of-oblivion-role-${code}`);
  return v === "p1" || v === "p2" ? v : null;
}

function storeRole(code: string, role: Seat) {
  window.localStorage.setItem(`mine-of-oblivion-role-${code}`, role);
}

/**
 * Whose decision `useBotAutoplay` should drive right now. `SETUP_MINE` is
 * simultaneous-by-rule (both seats secretly place independently, no turn
 * order) — same "lowest-numbered seat that still hasn't acted" convention
 * grid-poker's `gridPokerCurrentActor` uses for its own simultaneous phases.
 * `REVEAL_STEP` deliberately returns null — advancing past it is the host's
 * shared-clock timer below, not any one seat's move.
 */
function mooCurrentActor(state: MineOfOblivionState): Seat | null {
  if (state.phase === "SETUP_MINE") {
    if (!state.mineReady.p1) return "p1";
    if (!state.mineReady.p2) return "p2";
    return null;
  }
  if (state.phase === "PLAYER_MOVE") return state.activeSeat;
  return null;
}

export default function MineOfOblivionGame({ onComplete }: PlayableGameProps) {
  ...
  const [gameState, setGameState] = useState<MineOfOblivionState | null>(null);

  // Host-controlled AI bot roster (ARCHITECTURE.md §7) — same shape as every other online game here.
  ...
  // Only 1 other seat can ever exist to vote in this 2-player game, so a vote "passes" the instant that one opponent votes yes.
  ...
  const gameStateRef = useRef<MineOfOblivionState | null>(null);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`mine-of-oblivion-room-${roomCode}`, {
    ...
    channel.on("broadcast", { event: "state-sync" }, ({ payload }) => {
      const syncedState = payload?.state as MineOfOblivionState | undefined;
  ...
  // Cheap synchronous heuristic (engine.ts's `scoreMove`/`chooseBotAction`) — no search deep enough to warrant a Web Worker.
  const chooseAction = useCallback((state: MineOfOblivionState, actor: Seat): EngineAction | null => {
  ...
  useBotAutoplay<MineOfOblivionState, EngineAction, Seat>({
    active: isHost && phase === "playing",
    state: gameState,
    currentActor: mooCurrentActor,
    botSeats: allBotRoleSet,
    chooseAction,
    dispatch: handleAction,
    // 룰북 확인 답변의 "봇은 1.5초 내 자동 이동/선택" 요구를 그대로 반영 —
    // 다른 게임들의 기본값(500–1500ms)과 동일한 상한.
    minDelayMs: 500,
    maxDelayMs: 1500,
  });

  // REVEAL_STEP's shared clock — the host alone advances it once REVEAL_SECONDS
  // has elapsed, exactly like grid-poker's round-result timer (see this
  // file's module doc). Any client's Board.tsx skip button can also fire the
  // same action early; whichever broadcast lands first wins, the other is a
  // harmless no-op once `phase` has already moved on.
  useEffect(() => {
    if (!isHost || phase !== "playing" || !gameState) return;
    if (gameState.phase !== "REVEAL_STEP") return;
    const id = setTimeout(() => {
      handleAction({ type: "READY_NEXT_ROUND" });
    }, REVEAL_SECONDS * 1000);
    return () => clearTimeout(id);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- keyed to this reveal episode itself (actionsPlayed is stable for the whole REVEAL_STEP), same pattern as grid-poker's round-result timer
  }, [isHost, phase, gameState?.phase, gameState?.actionsPlayed]);
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      if (!state) return;
      const actor = mooCurrentActor(state);
  ...
  // Report the finished match up to the betting/history layer the instant
  // every client's replayed state reaches GAME_OVER.
  useEffect(() => {
    if (!gameState || gameState.phase !== "GAME_OVER" || completedRef.current) return;
    ...
    const rankings = gameState.isDraw
      ...
      : [
          { playerId: gameState.winner === "p1" ? ids.p1 : ids.p2, rank: 1 },
          { playerId: gameState.winner === "p1" ? ids.p2 : ids.p1, rank: 2 },
        ];
  ...
  // best-effort, see useActiveRoomListing.ts. Mine of Oblivion is a fixed
  // 2-seat (p1/p2) game, so maxPlayers is always 2.
  useActiveRoomListing({
    gameId: "mine-of-oblivion",
    roomCode,
    isHost,
  ...
  function withGuard(node: ReactNode) {
    return (
      <>
        ...
        {showRulebook && <MineOfOblivionRulebookModal onClose={() => setShowRulebook(false)} />}
      </>
    );
  }
    ...
    return withGuard(
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-8 text-center">
        ...
        <p className="max-w-sm text-sm text-amber-100/80">
          망각의 지뢰는 실시간 온라인 대전 전용이라 Supabase 설정이 필요합니다.{" "}
  ...
  if (phase === "choose") {
    return withGuard(
      <RulebookGate
        gameId="mine-of-oblivion"
        containerClassName="border-white/10 bg-gradient-to-b from-[#1a0a0e] via-[#12070a] to-black light:border-slate-200 light:bg-none light:bg-white"
        icon="💣"
        title="망각의 지뢰 온라인 대전"
        description={<p className="text-sm text-white/50 light:text-slate-500">두 사람이 각자 기기로 접속해서 실시간으로 플레이해요.</p>}
        actions={
  ...
  if (phase === "enter-name") {
    return withGuard(
      <div className="flex flex-col gap-4 rounded-2xl border border-white/10 light:border-slate-200 bg-gradient-to-b from-[#1a0a0e] via-[#12070a] to-black p-6">
        ...
        {intent === "join" && (
          <label className="flex flex-col gap-1.5 text-sm text-white/70 light:text-slate-700">
            ...
            <input
              ...
              placeholder="0000"
              inputMode="numeric"
              className="rounded-lg border border-white/10 light:border-slate-200 bg-white/5 light:bg-slate-900/5 px-3 py-2 text-center text-lg font-semibold tracking-[0.3em] text-white light:text-slate-900 placeholder:text-white/20 focus:border-rose-400 focus:outline-none"
            />
          </label>
        )}
        ...
        <div className="flex gap-2">
          ...
          <button onClick={enterRoom} className="flex-1 rounded-xl bg-rose-500 py-2.5 text-sm font-semibold text-white hover:bg-rose-400">
  ...
  if (phase === "connecting" || phase === "waiting") {
    return withGuard(
      <div className="flex flex-col items-center gap-5 rounded-2xl border border-white/10 light:border-slate-200 bg-gradient-to-b from-[#1a0a0e] via-[#12070a] to-black p-8 text-center">
        {phase === "connecting" ? (
  ...
  if (phase === "playing" && gameState && myRole) {
    ...
    return withGuard(
      <>
        ...
        <MineOfOblivionBoard state={gameState} viewerSeat={myRole} names={names} opponentConnected={opponentConnected} onAction={handleAction} onLeave={handleLeave} onRematch={handleRematch} />
      </>,
    );
  }
