...
/**
 * Small local system-log formatter for the "playCard" headline action (see
 ...
 * "지수님이 경비병 카드를 냈습니다".
 */
function formatLoveLetterPlayCardLog(name: string, cardName: string): string {
  return `${name}님이 ${cardName} 카드를 냈습니다`;
}

/**
 * Online-room multiplayer entry point — same lockstep pattern as every other
 * `<Game>Game.tsx` in this project (closely modeled on
 * coyote/CoyoteGame.tsx, itself modeled on dalmuti/DalmutiGame.tsx): every
 * connected client independently computes the full `LoveLetterState`
 * (every seat's actual hand) from a shared RNG seed plus replayed
 ...
 * only (`LoveLetterBoard.tsx`'s use of `getPlayerView`). No house-rule
 * toggle here — the rulebook has no optional variant to expose beyond the
 * single-round-vs-token-loop choice already resolved in engine.ts's module
 * doc.
 */
...
function getStoredSeat(code: string): number | null {
  const v = window.localStorage.getItem(`love-letter-seat-${code}`);
...
function storeSeat(code: string, seat: number) {
  window.localStorage.setItem(`love-letter-seat-${code}`, String(seat));
}
...
export default function LoveLetterGame({ onComplete }: PlayableGameProps) {
  ...
  const [gameState, setGameState] = useState<LoveLetterState | null>(null);
  ...
  const gameStateRef = useRef<LoveLetterState | null>(null);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`love-letter-room-${roomCode}`, {
      config: { broadcast: { self: true }, presence: { key: deviceId } },
    });
    channelRef.current = channel;
    startSentRef.current = false;

    const chatChannel = `room:love-letter:${roomCode}`;
    ...
    channel.on("broadcast", { event: "game-action" }, ({ payload }) => {
      ...
      if (action.type === "playCard") {
        ...
        const card = prevState?.players.find((p) => p.seat === action.seat)?.hand.find((c) => c.id === action.cardId);
        if (card) {
          setChatMessages((prev) => [
            ...prev,
            {
              ...
              deviceId: "system",
              senderName: "시스템",
              body: formatLoveLetterPlayCardLog(namesRef.current[action.seat] ?? "상대", CARD_NAMES[card.number]),
              type: "SYSTEM",
    ...
    channel.on("broadcast", { event: "state-sync" }, ({ payload }) => {
      const state = payload?.state as LoveLetterState | undefined;
      if (!state) return;
      const roster = (payload?.botSeats as SeatIndex[] | undefined) ?? [];
      const levels = (payload?.botLevels as BotLevel[] | undefined) ?? [];
  ...
  const sendChatMessage = useCallback(
    (rawBody: string): SendResult => {
      ...
      const message: ChatMessage = {
        id: uuid(),
        channel: `room:love-letter:${roomCode}`,
        deviceId,
  ...
  const chooseAction = useCallback((state: LoveLetterState, actor: SeatIndex): EngineAction | null => {
  ...
  useBotAutoplay<LoveLetterState, EngineAction, SeatIndex>({
    active: isHost && phase === "playing",
    state: gameState,
    currentActor: loveLetterCurrentActor,
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      if (!state) return;
      const actor = loveLetterCurrentActor(state);
    ...
    return withGuard(
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-8 text-center">
        ...
        <p className="max-w-sm text-sm text-amber-100/80">
          러브레터는 실시간 온라인 대전 전용이라 Supabase 설정이 필요합니다.
          <code className="mx-1 rounded bg-black/30 light:bg-white/80 light:shadow-sm px-1.5 py-0.5 text-xs">.env.local</code>
          에 <code className="rounded bg-black/30 light:bg-white/80 light:shadow-sm px-1.5 py-0.5 text-xs">NEXT_PUBLIC_SUPABASE_URL</code> /
          <code className="mx-1 rounded bg-black/30 light:bg-white/80 light:shadow-sm px-1.5 py-0.5 text-xs">NEXT_PUBLIC_SUPABASE_ANON_KEY</code>
          를 채워주세요 (README 참고).
        </p>
      </div>
    );
      ...
      <RulebookGate
        ...
        description={
          <p className="text-sm text-white/50 light:text-slate-500">
            {MIN_PLAYERS}~{MAX_PLAYERS}명이 각자 기기로 접속해서 실시간으로 플레이해요. 단판 승부로 공주의 마음을 얻어보세요!
          </p>
        }
        actions={
  ...
  if (phase === "enter-name") {
    return withGuard(
      <div className="flex flex-col gap-4 rounded-2xl border border-white/10 light:border-slate-200 bg-white/[0.03] p-6">
          ...
          <label className="flex flex-col gap-1.5 text-sm text-white/70 light:text-slate-700">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="h-8 w-8 rounded-full border border-white/15 light:border-slate-200 text-white/80 light:text-slate-700 hover:border-white/30"
              >
                −
              </button>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={enterRoom} className="flex-1 rounded-xl bg-rose-600 py-2.5 text-sm font-semibold text-white hover:bg-rose-500">
  ...
  if (phase === "connecting" || phase === "waiting") {
    return withGuard(
      <>
      <div className="flex flex-col items-center gap-5 rounded-2xl border border-white/10 light:border-slate-200 bg-white/[0.03] p-8 text-center">
        {phase === "connecting" ? (
          ...
          <>
            ...
            <div className="mt-2 flex flex-col gap-1.5">
              {Array.from({ length: knownTargetPlayerCount }, (_, seat) => {
                ...
                return (
                  <p key={seat} className="flex items-center justify-between gap-2 text-sm text-white/70 light:text-slate-700">
                    <span>
            ...
            {isHost && occupants.length + botSeats.length >= MIN_PLAYERS && occupants.length + botSeats.length < knownTargetPlayerCount && (
              <button onClick={sendGameStart} className="rounded-full bg-rose-600 px-4 py-2 text-xs font-semibold text-white hover:bg-rose-500">
  ...
  if (phase === "playing" && gameState && mySeat !== null) {
    ...
    return withGuard(
      ...
      <LoveLetterBoard state={gameState} viewerSeat={mySeat} names={names} connectedSeats={connectedSeats} onAction={handleAction} onGameEnd={handleGameEnd} />
  ...
  if (phase === "post-game" && finalResult) {
    return withGuard(
      ...
      <div className="flex flex-col items-center gap-5 rounded-2xl border border-white/10 light:border-slate-200 bg-white/[0.03] p-8 text-center">
        ...
        <p className="text-white/80 light:text-slate-700">{finalResult.winnerNames} 님이 공주의 마음을 얻어 게임이 끝났어요.</p>
        <div className="flex gap-2">
          <button onClick={handleLeave} className="rounded-xl border border-white/15 light:border-slate-200 px-4 py-2.5 text-sm text-white/70 light:text-slate-700 hover:border-white/30">
            나가기
          </button>
          <button onClick={handleRematch} className="rounded-xl bg-rose-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-rose-500">
            다시하기
          </button>
        </div>
      </div>
