    ...
    <Overlay title="🕓 직전 라운드 결과" onClose={onClose} wide>
      {!round ? (
        ...
        <div className="flex flex-col gap-5 text-sm text-white/80 light:text-slate-700">
          ...
          <div className="flex flex-wrap items-center gap-3 rounded-xl border border-white/10 bg-white/[0.02] p-3 light:border-slate-200 light:bg-white/90 light:shadow-sm">
            <span className="text-xs text-white/50 light:text-slate-500">ROUND {round.roundNumber}</span>
            <span className="font-semibold text-white/90 light:text-slate-900">🛡️ {names[round.challengerSeat]}</span>
            {round.outcome === "success" ? (
              <span className="rounded-full border border-emerald-400/50 bg-emerald-400/10 px-2 py-0.5 text-xs font-bold text-emerald-200 light:text-emerald-700">
                ✅ 공략 성공
              </span>
            ) : (
              <span className="rounded-full border border-rose-400/50 bg-rose-400/10 px-2 py-0.5 text-xs font-bold text-rose-200 light:text-rose-700">
                💀 공략 실패
              </span>
            )}
            {round.newlyEliminated && (
              <span className="rounded-full border border-rose-400/50 bg-rose-400/10 px-2 py-0.5 text-xs font-bold text-rose-200 light:text-rose-700">🪦 탈락</span>
            )}
            <span className="ml-auto text-xs text-white/60 light:text-slate-600">
              ❤️ 남은 체력 <b className="text-white light:text-slate-900">{Math.max(0, round.finalHp)}</b> / 최대 체력 <b className="text-white light:text-slate-900">{round.totalHp}</b>
            </span>
          </div>

          {round.spatulaDeclaredThreat !== null && (
            <p className="text-xs text-white/50 light:text-slate-500">🥄 황금 뒤집개 지정 몬스터: 위협도 {round.spatulaDeclaredThreat}</p>
          )}

          {/* 2-a. 제외된 장비 목록 — 플레이어별 */}
          <div className="flex flex-col gap-2">
            <h3 className="text-xs font-semibold tracking-wide text-white/50 uppercase light:text-slate-500">🗑️ 제외된 장비</h3>
            {round.removedByPlayer.length === 0 ? (
              <p className="text-xs text-white/35 light:text-slate-400">이번 라운드엔 아무도 장비를 제외하지 않았습니다.</p>
            ) : (
              <div className="flex flex-col gap-2">
                {round.removedByPlayer.map(({ seat, itemIds }) => (
                  <div key={seat} className="flex flex-col gap-1">
                    <span className="text-xs font-medium text-white/70 light:text-slate-600">{names[seat]}</span>
          ...
          <div className="flex flex-col gap-2">
            <h3 className="text-xs font-semibold tracking-wide text-white/50 uppercase light:text-slate-500">📜 등장 몬스터 & 격파 과정</h3>
            {round.combatLog.length === 0 ? (
              <p className="text-xs text-white/35 light:text-slate-400">협곡 더미가 비어 있어 몬스터 없이 바로 공략에 성공했습니다.</p>
            ) : (
              <div className="flex flex-wrap gap-2.5">
                {round.combatLog.map((entry, i) => (
                  <div key={entry.monster.id} className="flex flex-col items-center gap-1">
                    <span className="text-[10px] text-white/40 light:text-slate-500">#{i + 1}</span>
