...
import SummonersRiftBoard from "./SummonersRiftBoard";
...
/** Whose decision `useBotAutoplay` should drive right now — the one seat with a pending action across bidding/declaringSpatula/resolvingRift, or null once the round has ended and there's nothing left for any seat to decide. */
function summonersRiftCurrentActor(state: SummonersRiftState): SeatIndex | null {
  if (state.phase === "bidding") return state.pendingDraw ? state.pendingDraw.seat : state.activeSeat;
  if (state.phase === "declaringSpatula" || state.phase === "resolvingRift") return state.challengerSeat;
  return null;
}

/**
 * System-log pilot (see GameMeta.chatEnabled, PerudoGame.tsx/DalmutiGame.tsx)
 * — headline action is `revealNextMonster`, the core "협곡 공략" combat step:
 * the challenger flips the next Rift-pile monster and either kills it for
 * free (an equipped item, or the declared golden-spatula target) or takes
 * damage from it.
 */
function formatSummonersRiftRevealLog(name: string, monsterName: string, killedForFree: boolean, damage: number): string {
  return killedForFree
    ? `${name}님이 ${monsterName}을(를) 아이템으로 처치했습니다`
    : `${name}님이 ${monsterName}에게 데미지 ${damage}를 입었습니다`;
}
 ...
 * `SummonersRiftState` (including the exact deck order and every card's
 * identity) from a shared RNG seed plus replayed `EngineAction`s broadcast
 * over Supabase Realtime — there is no server-authoritative engine. See
 * engine.ts and docs/architecture.md §2 for the accepted trust trade-off.
 */
...
function getStoredSeat(code: string): number | null {
  const v = window.localStorage.getItem(`summoners-rift-seat-${code}`);
...
function storeSeat(code: string, seat: number) {
  window.localStorage.setItem(`summoners-rift-seat-${code}`, String(seat));
}
...
export default function SummonersRiftGame({ onComplete }: PlayableGameProps) {
  ...
  const [gameState, setGameState] = useState<SummonersRiftState | null>(null);
  ...
  // DalmutiGame.tsx/CoyoteGame.tsx). Every client independently replays the
  // same broadcast `BotTakeoverEvent`s; no host dependency.
  ...
  const gameStateRef = useRef<SummonersRiftState | null>(null);
  useEffect(() => {
  ...
  function enterRoom() {
    ...
    const name = identity.name.trim() || "소환사";
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`summoners-rift-room-${roomCode}`, {
      config: { broadcast: { self: true }, presence: { key: deviceId } },
    });
    channelRef.current = channel;
    startSentRef.current = false;

    const chatChannel = `room:summoners-rift:${roomCode}`;
    ...
    channel.on("broadcast", { event: "game-action" }, ({ payload }) => {
      ...
      // user messages) — every client would otherwise write a duplicate row,
      // and it's trivially re-derivable from the replayed action log anyway.
      if (action.type === "revealNextMonster") {
        const prevState = gameStateRef.current;
        const monster = prevState?.riftPile[0];
        if (prevState && monster) {
          const killer = findKiller(prevState.equippedItemIds, prevState.spatulaDeclaredThreat, monster.threat);
          setChatMessages((prev) => [
            ...prev,
            {
              ...
              deviceId: "system",
              senderName: "시스템",
              body: formatSummonersRiftRevealLog(
                namesRef.current[action.seat] ?? "상대",
                getMonsterDef(monster.threat).name,
                killer !== null,
                killer ? 0 : monster.threat,
              ),
              type: "SYSTEM",
    ...
    channel.on("broadcast", { event: "state-sync" }, ({ payload }) => {
      const state = payload?.state as SummonersRiftState | undefined;
      if (!state) return;
      const roster = (payload?.botSeats as SeatIndex[] | undefined) ?? [];
      const levels = (payload?.botLevels as BotLevel[] | undefined) ?? [];
  ...
  const sendChatMessage = useCallback(
    (rawBody: string): SendResult => {
      ...
      const message: ChatMessage = {
        id: uuid(),
        channel: `room:summoners-rift:${roomCode}`,
        deviceId,
  ...
  // `botSeats` roster wherever bot-seat membership matters. Depending on a
  ...
  // taken-over seats* itself actually changes — see DalmutiGame.tsx's
  ...
  const chooseAction = useCallback((state: SummonersRiftState, actor: SeatIndex): EngineAction | null => {
  ...
  useBotAutoplay<SummonersRiftState, EngineAction, SeatIndex>({
    active: isHost && phase === "playing",
    state: gameState,
    currentActor: summonersRiftCurrentActor,
  ...
  // that seat (unless it's already a bot or already mid-vote).
  useEffect(() => {
    if (phase !== "playing") return;
    const interval = window.setInterval(() => {
      const state = gameStateRef.current;
      if (!state) return;
      const actor = summonersRiftCurrentActor(state);
    ...
    return withGuard(
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-8 text-center light:bg-amber-50 light:border-amber-300 light:shadow-sm">
        ...
        <p className="max-w-sm text-sm text-amber-100/80 light:text-amber-800">
          소환사의 협곡은 실시간 온라인 대전 전용이라 Supabase 설정이 필요합니다.
  ...
  if (phase === "enter-name") {
    return withGuard(
      <div className="flex flex-col gap-4 rounded-2xl border border-white/10 bg-white/[0.03] p-6 light:bg-white/90 light:border-slate-200 light:shadow-sm">
        ...
        {formError && <p className="rounded-lg bg-rose-500/10 px-3 py-2 text-xs text-rose-300 light:bg-rose-100 light:text-rose-700">{formError}</p>}
        <div className="flex gap-2">
          <button onClick={() => setPhase("choose")} className="flex-1 rounded-xl border border-white/15 py-2.5 text-sm text-white/70 hover:border-white/30 light:border-slate-300 light:text-slate-600 light:hover:border-slate-400">
            뒤로
          </button>
      ...
      <div className="flex flex-col items-center gap-5 rounded-2xl border border-white/10 bg-white/[0.03] p-8 text-center light:bg-white/90 light:border-slate-200 light:shadow-sm">
        {phase === "connecting" ? (
          ...
          <>
            ...
            <button onClick={() => navigator.clipboard?.writeText(shareUrl)} className="rounded-full border border-white/15 px-4 py-2 text-xs text-white/70 hover:border-white/30 light:border-slate-300 light:text-slate-600 light:hover:border-slate-400">
              🔗 초대 링크 복사
            </button>
  ...
  if (phase === "playing" && gameState && mySeat !== null) {
    ...
    return withGuard(
      ...
      <SummonersRiftBoard state={gameState} viewerSeat={mySeat} names={names} connectedSeats={connectedSeats} onAction={handleAction} onGameEnd={handleGameEnd} />
  ...
  if (phase === "post-game" && finalResult) {
    return withGuard(
      ...
      <div className="flex flex-col items-center gap-5 rounded-2xl border border-white/10 bg-white/[0.03] p-8 text-center light:bg-white/90 light:border-slate-200 light:shadow-sm">
        ...
        <p className="text-white/80 light:text-slate-700">{finalResult.winnerName} 님이 협곡의 최종 승자가 되어 게임이 끝났어요.</p>
        <div className="flex gap-2">
          <button onClick={handleLeave} className="rounded-xl border border-white/15 px-4 py-2.5 text-sm text-white/70 hover:border-white/30 light:border-slate-300 light:text-slate-600 light:hover:border-slate-400">
            나가기
          </button>
