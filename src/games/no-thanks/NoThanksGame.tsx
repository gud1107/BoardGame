  ...
  if (phase === "room-full") {
    return withGuard(
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-rose-400/30 bg-rose-400/10 p-8 text-center light:border-rose-300 light:bg-rose-50">
        ...
        <p className="text-sm text-rose-100/80 light:text-rose-600">코드를 다시 확인하거나 새로운 방을 만들어보세요.</p>
        <button
          onClick={handleLeave}
  ...
  if (phase === "choose") {
    return withGuard(
      <RulebookGate
        ...
        icon="🙅"
        title="노땡스! 온라인 대전"
        description={<p className="text-sm text-white/50 light:text-slate-500">3~7명이 각자 기기로 접속해서 실시간으로 플레이해요.</p>}
        actions={
        ...
        {intent === "join" && (
          <label className="flex flex-col gap-1.5 text-sm text-white/70 light:text-slate-600">
            ...
            <input
              ...
              placeholder="0000"
              inputMode="numeric"
              className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-center text-lg font-semibold tracking-[0.3em] text-white placeholder:text-white/20 focus:border-emerald-400 focus:outline-none light:border-slate-300 light:bg-white light:text-slate-900 light:placeholder:text-slate-300"
            />
          </label>
        )}
        {intent === "create" && (
          ...
          <div className="flex flex-col gap-1.5 text-sm text-white/70 light:text-slate-600">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 rounded-xl border px-3 py-2 text-left text-xs transition ${
                  chipVisibility === "secret"
                    ? "border-emerald-400/60 bg-emerald-400/10 text-emerald-100 light:bg-emerald-50 light:text-emerald-800"
              ...
              >
                ...
                <p className="text-white/50 light:text-slate-500">공식 룰 · 내 칩만 나에게 보임</p>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <p className="text-white/50 light:text-slate-500">커스텀 룰 · 모두의 칩이 공개됨</p>
              </button>
            </div>
          </div>
        )}
