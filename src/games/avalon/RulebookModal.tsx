      ...
      <div className="flex flex-col gap-5 text-sm text-white/80 light:text-slate-700">
        <section>
          ...
          <p className="text-white/70 light:text-slate-600">
            <span className="text-sky-300 light:text-sky-700">선한 세력</span>은 원정(퀘스트)을 3회 성공시키고, 암살자가 메를린을 맞히지
            못해야 최종 승리합니다. <span className="text-rose-300 light:text-rose-700">악한 세력</span>은 원정을 3회 실패시키거나, 원정대
            지목이 연속 5회 부결되거나, 원정 3회가 성공해도 암살자가 메를린을 정확히 지목하면 승리합니다.
          </p>
        </section>

        <section>
          <h3 className="mb-2 text-xs font-semibold tracking-wide text-white/50 uppercase light:text-slate-500">역할</h3>
        ...
        <section>
          <h3 className="mb-2 text-xs font-semibold tracking-wide text-white/50 uppercase light:text-slate-500">인원별 세력 구성</h3>
            ...
            <table className="w-full min-w-[360px] border-collapse text-xs">
              <thead>
                <tr className="text-white/50 light:text-slate-500">
                  ...
                  <th className="border-b border-white/10 px-2 py-1.5 text-left light:border-slate-200">선</th>
                  <th className="border-b border-white/10 px-2 py-1.5 text-left light:border-slate-200">악</th>
                </tr>
              </thead>
              <tbody>
                {COMPOSITION.map((row) => (
                  <tr key={row.count}>
                    ...
                    <td className="border-b border-white/5 px-2 py-1.5 text-sky-300 light:border-slate-100 light:text-sky-700">{row.good}</td>
                    <td className="border-b border-white/5 px-2 py-1.5 text-rose-300 light:border-slate-100 light:text-rose-700">{row.evil}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section>
          <h3 className="mb-2 text-xs font-semibold tracking-wide text-white/50 uppercase light:text-slate-500">원정 참여 인원 (1~5라운드)</h3>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[300px] border-collapse text-xs">
              <thead>
                <tr className="text-white/50 light:text-slate-500">
                  <th className="border-b border-white/10 px-2 py-1.5 text-left light:border-slate-200">인원</th>
                  <th className="border-b border-white/10 px-2 py-1.5 text-left light:border-slate-200">라운드별 인원수</th>
                </tr>
              </thead>
              <tbody>
                {TEAM_SIZES.map((row) => (
                  <tr key={row.count}>
                    <td className="border-b border-white/5 px-2 py-1.5 text-white light:border-slate-100 light:text-slate-900">{row.count}</td>
                    <td className="border-b border-white/5 px-2 py-1.5 text-white/70 light:border-slate-100 light:text-slate-600">{row.sizes}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section>
          ...
          <p className="text-white/70 light:text-slate-600">
            ...
            동수는 부결) → ③ 부결되면 리더가 시계방향으로 넘어가 다시 지목 (한 라운드에서 <span className="text-rose-300 light:text-rose-700">연속 5회</span>{" "}
        ...
        <section>
          <h3 className="mb-2 text-xs font-semibold tracking-wide text-white/50 uppercase light:text-slate-500">암살 단계</h3>
          <p className="text-white/70 light:text-slate-600">
