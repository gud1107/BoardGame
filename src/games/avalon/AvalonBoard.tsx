...
const TEAM_LABEL: Record<Team, string> = { good: "선한 세력 승리!", evil: "악한 세력 승리!" };
...
export default function AvalonBoard({ state, viewerSeat, names, connectedSeats, onAction, onGameEnd }: AvalonBoardProps) {
  ...
  const [trackedRoundPhaseKey, setTrackedRoundPhaseKey] = useState(roundPhaseKey);
  ...
  const assassinSeat = useMemo(() => state.players.find((p) => p.role === "assassin")?.seat ?? null, [state.players]);
  ...
  if (state.phase === "gameOver" && state.winner) {
    return (
      <div className={`${TABLE_PANEL} flex flex-col items-center gap-5 p-8 text-center`}>
        ...
        <p className="relative z-10 text-sm text-white/60">{WIN_REASON_LABEL[state.winReason!]}</p>
            ...
            return (
              <div
                ...
                className={`flex flex-col items-center gap-1 rounded-xl border p-2 text-xs ${
                  meta.team === "good" ? "border-sky-400/40 bg-sky-400/10" : "border-rose-400/40 bg-rose-400/10"
                }`}
              >
  ...
  const otherSeats = Array.from({ length: state.playerCount }, (_, i) => i).filter((s) => s !== viewerSeat);
  ...
  return (
    <div className={`${TABLE_PANEL} flex flex-col gap-3 p-3 sm:p-4`}>
      ...
      <div className="relative z-10 flex flex-wrap items-center justify-between gap-1.5 text-xs text-amber-100/60">
        <span>
          {state.playerCount}인 · {state.round}라운드/5 · 원정대 {teamSize}명 필요
          {threshold > 1 ? ` · 이번 라운드는 실패 ${threshold}장 이상이어야 실패` : ""}
        </span>
      ...
      <div className="relative z-10 rounded-xl border border-white/10 bg-black/30 p-3 text-center">
          ...
          (isLeader ? (
            <div className="flex flex-col gap-2">
              <p className="text-xs text-amber-100/80">
                원정대 {teamSize}명을 골라주세요 ({selection.length}/{teamSize} 선택됨) — 좌석을 눌러 선택하세요.
              </p>
              <button
            ...
            <p className="text-xs text-amber-100/50">{names[state.leader]}님이 원정대를 고르는 중...</p>
          ))}

        {state.phase === "voting" &&
          (iVoted ? (
            <p className="text-xs text-amber-100/60">
              투표 완료 · {votedCount}/{state.playerCount}명 투표함 — 다른 사람을 기다리는 중...
            </p>
          ) : (
            <div className="flex flex-col gap-2">
              <p className="text-xs text-amber-100/80">
                원정대: {state.proposedTeam.map((s) => names[s]).join(", ")} — 찬성하시겠어요?
              </p>
        ...
        {state.phase === "quest" &&
          (!iAmOnTeam ? (
            <p className="text-xs text-amber-100/50">
              원정대({state.proposedTeam.map((s) => names[s]).join(", ")})가 카드를 제출하는 중... ({questSubmittedCount}/
              {teamSize})
            </p>
          ) : iSubmittedQuestCard ? (
            <p className="text-xs text-amber-100/60">
              제출 완료 · {questSubmittedCount}/{teamSize}명 제출함 — 다른 원정대원을 기다리는 중...
            </p>
          ) : viewer.team === "good" ? (
            <div className="flex flex-col items-center gap-2">
              <p className="text-xs text-amber-100/80">선한 세력은 무조건 성공 카드만 제출할 수 있어요.</p>
              <button
            ...
            <div className="flex flex-col items-center gap-2">
              <p className="text-xs text-amber-100/80">원정 카드를 비공개로 제출하세요.</p>
        ...
        {state.phase === "assassination" &&
          (isAssassin ? (
            <div className="flex flex-col gap-2">
              <p className="text-xs text-amber-100/80">
                원정 3회 성공! 메를린으로 의심되는 사람을 지목해 암살하세요. 정확히 맞히면 악한 세력이 역전 승리합니다.
              </p>
            ...
            <p className="text-xs text-amber-100/50">
              원정 3회 성공! {assassinSeat !== null ? names[assassinSeat] : "암살자"}님이 메를린을 지목하는 중...
            </p>
          ))}
      </div>
      ...
      {roleModalOpen && (
        <RoleModal viewerSeat={viewerSeat} names={names} viewer={viewer} knowledge={knowledge} onClose={() => setRoleModalOpen(false)} />
      )}
    </div>
  );
}
...
function SeatChip({
  ...
  const isSelectable = state.phase === "team-proposal" && state.leader !== undefined;
  const isSelected = selection.includes(seat);
  const votedAlready = state.phase === "voting" && state.votes[seat] !== undefined;
  const questSubmittedAlready =
    state.phase === "quest" && state.proposedTeam.includes(seat) && state.questCards[seat] !== undefined;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="w-full max-w-sm rounded-2xl border border-amber-400/30 bg-[#1a1128] p-6 text-center shadow-2xl">
        <p className="mb-1 text-xs text-white/40">{names[viewerSeat]}님의 비밀 정보</p>
        <div className="mb-3 text-5xl">{meta.icon}</div>
        <h2 className={`mb-1 text-xl font-bold ${meta.team === "good" ? "text-sky-300" : "text-rose-300"}`}>
          {meta.label}
        </h2>
        ...
        {knowledge.merlinPercivalCandidates.length > 0 && (
          <div className="mb-3 rounded-xl border border-sky-400/30 bg-sky-500/10 p-3 text-sm text-sky-100">
            ...
            <p>{knowledge.merlinPercivalCandidates.map((s) => names[s]).join(", ")}</p>
            <p className="mt-1 text-xs text-sky-200/60">둘 중 누가 진짜 메를린인지는 알 수 없습니다.</p>
          </div>
        )}

        {knowledge.evilSeatsKnown.length === 0 && knowledge.merlinPercivalCandidates.length === 0 && (
          <p className="mb-3 text-xs text-white/40">이 역할은 다른 사람에 대한 추가 정보가 없습니다.</p>
        )}
