 ...
 * importing engine.ts. "vote" is Avalon's most game-defining action: every
 * seat casts one on every proposed team, and it's the moment suspicion
 * actually gets expressed.
 */
function formatAvalonVoteLog(name: string, vote: "approve" | "reject"): string {
  return `${name}님이 원정대 제안에 ${vote === "approve" ? "찬성" : "반대"}했습니다`;
}

/**
 * Whose decision `useBotAutoplay` should drive right now. `voting` (every
 * seat decides at once) and `quest` (every proposed-team seat decides at
 * once) have no single active seat — same "lowest still-undecided seat"
 * workaround every other simultaneous-decision phase in this project uses.
 */
function avalonCurrentActor(state: AvalonState): SeatIndex | null {
 ...
 * BangGame/GridPokerGame: every connected client independently computes the
 * full `AvalonState` (every seat's secret role) from a shared RNG seed plus
 * replayed `EngineAction`s broadcast over Supabase Realtime — there is no
 * server-authoritative engine. See engine.ts and README for the accepted
 * trust trade-off (a technically inclined player could inspect their own
...
function getStoredSeat(code: string): number | null {
  const v = window.localStorage.getItem(`avalon-seat-${code}`);
...
function storeSeat(code: string, seat: number) {
  window.localStorage.setItem(`avalon-seat-${code}`, String(seat));
}
...
export default function AvalonGame({ onComplete }: PlayableGameProps) {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`avalon-room-${roomCode}`, {
      config: { broadcast: { self: true }, presence: { key: deviceId } },
    });
    channelRef.current = channel;
    startSentRef.current = false;

    const chatChannel = `room:avalon:${roomCode}`;
      ...
      if (action.type === "vote") {
        setChatMessages((prev) => [
          ...prev,
          {
            ...
            deviceId: "system",
            senderName: "시스템",
            body: formatAvalonVoteLog(namesRef.current[action.seat] ?? "상대", action.vote),
            type: "SYSTEM",
  ...
  // instant — same self-healing tie-break as BangGame.tsx: whichever device
  // has the lexicographically larger id gives up the seat and claims the
  // next free one, up to a few attempts before falling back to "room-full".
  useEffect(() => {
  ...
  const chooseAction = useCallback((state: AvalonState, actor: SeatIndex): EngineAction | null => {
  ...
  useBotAutoplay<AvalonState, EngineAction, SeatIndex>({
    active: isHost && phase === "playing",
    state: gameState,
    currentActor: avalonCurrentActor,
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      if (!state) return;
      const actor = avalonCurrentActor(state);
    ...
    return withGuard(
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-8 text-center light:bg-amber-50 light:border-amber-300 light:shadow-sm">
        ...
        <p className="max-w-sm text-sm text-amber-100/80 light:text-slate-600">
          아발론은 실시간 온라인 대전 전용이라 Supabase 설정이 필요합니다.
  ...
  if (phase === "choose") {
    return withGuard(
      <RulebookGate
        ...
        icon="🗡️"
        title="아발론 온라인 대전"
        description={<p className="text-sm text-white/50 light:text-slate-500">5~10명이 각자 기기로 접속해서 실시간으로 플레이해요.</p>}
        actions={
        ...
        {intent === "create" && (
          <label className="flex flex-col gap-1.5 text-sm text-white/70 light:text-slate-600">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                type="button"
                onClick={() => setTargetPlayerCount((n) => Math.min(10, n + 1))}
                className="h-8 w-8 rounded-full border border-white/15 text-white/80 hover:border-white/30 light:border-slate-300 light:text-slate-700"
              >
                +
              </button>
      ...
      <div className="flex flex-col items-center gap-5 rounded-2xl border border-white/10 bg-white/[0.03] p-8 text-center light:bg-white/90 light:border-slate-200 light:shadow-sm">
        {phase === "connecting" ? (
          ...
          <>
            ...
            {isHost && occupants.length + botSeats.length >= 5 && occupants.length + botSeats.length < knownTargetPlayerCount && (
              <button
    ...
    return withGuard(
      ...
      <div className="flex flex-col items-center gap-5 rounded-2xl border border-white/10 bg-white/[0.03] p-8 text-center light:bg-white/90 light:border-slate-200 light:shadow-sm">
        ...
        <p className="text-white/80 light:text-slate-700">
          {finalResult.winner === "good" ? "선한 세력" : "악한 세력"} 승리로 게임이 끝났어요.
        </p>
        <div className="flex gap-2">
          <button
            onClick={handleLeave}
