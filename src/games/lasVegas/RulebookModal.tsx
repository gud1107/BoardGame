      ...
      <div className="flex flex-col gap-5 text-sm text-white/80 light:text-slate-700">
        <section>
          ...
          <p className="text-white/70 light:text-slate-600">
            ...
            <span className="text-amber-300 light:text-amber-600">룰북 원문이 정식으로 채택한 단판(1라운드 완결) 모드</span>로 진행되며, 정산이 끝나는
            즉시 최종 승자가 가려집니다.
          </p>
        </section>

        <section>
          <h3 className="mb-2 text-xs font-semibold tracking-wide text-white/50 uppercase light:text-slate-500">세팅</h3>
          ...
          <div className="mt-2 overflow-x-auto rounded-xl border border-white/10 light:border-slate-200">
            <table className="w-full min-w-[420px] border-collapse text-xs">
              <thead>
                <tr className="bg-white/5 text-white/50 light:bg-slate-100 light:text-slate-500">
              ...
              <tbody>
                {NEUTRAL_ROWS.map((row) => (
                  <tr key={row.count} className="border-t border-white/10 light:border-slate-200">
                    <td className="px-2 py-1.5 font-semibold text-white/80 light:text-slate-800">{row.count}</td>
                    <td className="px-2 py-1.5 light:text-slate-700">{row.perPlayer}개</td>
                    <td className="px-2 py-1.5 text-white/50 light:text-slate-500">{row.note}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section>
          <h3 className="mb-2 text-xs font-semibold tracking-wide text-white/50 uppercase light:text-slate-500">내 턴에 하는 일</h3>
          <div className="grid gap-2 sm:grid-cols-3">
            <div className="rounded-xl border border-white/15 bg-white/[0.03] p-3 light:border-slate-200 light:bg-white light:shadow-sm">
              <p className="mb-1 font-medium text-white/90 light:text-slate-900">① 주사위 던지기</p>
              <p className="text-xs text-white/60 light:text-slate-500">손에 남은 모든 주사위(개인+중립)를 한 번에 굴립니다.</p>
            </div>
            <div className="rounded-xl border border-white/15 bg-white/[0.03] p-3 light:border-slate-200 light:bg-white light:shadow-sm">
              <p className="mb-1 font-medium text-white/90 light:text-slate-900">② 숫자 하나 선택</p>
              <p className="text-xs text-white/60 light:text-slate-500">굴린 눈금 중 단 하나의 숫자를 고릅니다.</p>
            </div>
            <div className="rounded-xl border border-white/15 bg-white/[0.03] p-3 light:border-slate-200 light:bg-white light:shadow-sm">
              <p className="mb-1 font-medium text-white/90 light:text-slate-900">③ 전량 배치</p>
        ...
        <section>
          <h3 className="mb-2 text-xs font-semibold tracking-wide text-white/50 uppercase light:text-slate-500">정산 — 동률 상쇄 규칙 (핵심!)</h3>
          <p className="text-white/70 light:text-slate-600">
            1번 카지노부터 순서대로 정산합니다. 카지노에 놓인 주사위 개수를 색상별(중립 포함)로 확인해{" "}
            <b>개수가 같은 색상들은 전부 무효화(상쇄)</b>되어 치워집니다.
          </p>
          <div className="my-2 flex flex-wrap items-center gap-2 rounded-xl border border-white/10 bg-black/20 p-2 text-xs light:border-slate-200 light:bg-slate-100">
            <span className="text-white/60 light:text-slate-500">예:</span>
            <DiceFace face={4} color="#ef4444" size="h-5 w-5" /> ×3
            <DiceFace face={4} color="#3b82f6" size="h-5 w-5" /> ×3
            <span className="text-rose-300 light:text-rose-600">→ 둘 다 상쇄</span>
        ...
        <section>
          <h3 className="mb-2 text-xs font-semibold tracking-wide text-white/50 uppercase light:text-slate-500">최종 승리</h3>
          <p className="text-white/70 light:text-slate-600">
        ...
        <section>
          <h3 className="mb-2 text-xs font-semibold tracking-wide text-white/50 uppercase light:text-slate-500">전략 팁</h3>
          <ul className="list-disc space-y-1 pl-4 text-xs text-white/60 light:text-slate-500">
