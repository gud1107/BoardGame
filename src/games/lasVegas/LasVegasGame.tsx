  ...
  if (phase === "choose") {
    return withGuard(
      <RulebookGate
        ...
        title="라스베가스 온라인 대전"
        description={
          <p className="text-sm text-white/50 light:text-slate-600">
    ...
    return withGuard(
      <div className="flex flex-col gap-4 rounded-2xl border border-white/10 bg-white/[0.03] p-6 light:border-slate-200 light:bg-white/90 light:shadow-sm">
        ...
        <div className="flex gap-2">
          <button onClick={() => setPhase("choose")} className="flex-1 rounded-xl border border-white/15 py-2.5 text-sm text-white/70 hover:border-white/30 light:border-slate-300 light:text-slate-700 light:hover:border-slate-400">
            뒤로
          </button>
      ...
      <div className="flex flex-col items-center gap-5 rounded-2xl border border-white/10 bg-white/[0.03] p-8 text-center light:border-slate-200 light:bg-white/90 light:shadow-sm">
        {phase === "connecting" ? (
          ...
          <>
            ...
            <button onClick={() => navigator.clipboard?.writeText(shareUrl)} className="rounded-full border border-white/15 px-4 py-2 text-xs text-white/70 hover:border-white/30 light:border-slate-300 light:text-slate-700 light:hover:border-slate-400">
              🔗 초대 링크 복사
            </button>
  ...
  if (phase === "post-game" && finalResult) {
    return withGuard(
      ...
      <div className="flex flex-col items-center gap-5 rounded-2xl border border-white/10 bg-white/[0.03] p-8 text-center light:border-slate-200 light:bg-white/90 light:shadow-sm">
        ...
        <p className="text-white/80 light:text-slate-800">
          {finalResult.winnerName}
          {finalResult.tied ? " 님 외 공동 우승으로 게임이 끝났어요." : " 님 최고 상금 획득으로 게임이 끝났어요."}
        </p>
        <div className="flex gap-2">
          <button onClick={handleLeave} className="rounded-xl border border-white/15 px-4 py-2.5 text-sm text-white/70 hover:border-white/30 light:border-slate-300 light:text-slate-700 light:hover:border-slate-400">
            나가기
          </button>
