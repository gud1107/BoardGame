 ...
 * `<Game>Game.tsx` in this project (closely modeled on no-thanks/splendor's
 * `NoThanksGame.tsx`/`SplendorGame.tsx`, see their header docs for the full
 * rationale): every connected client independently computes the full
 * `FiveCucumbersState` (every seat's hand) from a shared RNG seed plus
 ...
 * the accepted trust trade-off, and engine.ts's module doc for how hands
 * (secret by the physical rules) stay hidden at the UI layer only.
 */
...
type Occupant = {
  ...
  /** Host's choice, see engine.ts's `EliminationThreshold` — shown to waiting joiners so they know which house rule they're about to play. */
...
function getStoredSeat(code: string): number | null {
  const v = window.localStorage.getItem(`five-cucumbers-seat-${code}`);
...
function storeSeat(code: string, seat: number) {
  window.localStorage.setItem(`five-cucumbers-seat-${code}`, String(seat));
}
...
export default function FiveCucumbersGame({ onComplete }: PlayableGameProps) {
  ...
  // Task brief §1 "탈락 기준 선택 토글" — host-only choice made once before
  // the room fills up, applies identically to every seat (same pattern as
  // no-thanks's `ChipVisibility`). Both 5 and 6 are rulebook-sanctioned
  // (§4); only the default (5) came from the task brief, see engine.ts's doc.
  const [eliminationThreshold, setEliminationThreshold] = useState<EliminationThreshold>(DEFAULT_ELIMINATION_THRESHOLD);
  ...
  const gameStateRef = useRef<FiveCucumbersState | null>(null);
  useEffect(() => {
  ...
  function enterRoom() {
    ...
    eliminationThresholdRef.current = eliminationThreshold;
    setMyName(name);
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`five-cucumbers-room-${roomCode}`, {
    ...
    channel.on("broadcast", { event: "game-start" }, ({ payload }) => {
      ...
      const threshold = (payload?.eliminationThreshold as EliminationThreshold | undefined) ?? DEFAULT_ELIMINATION_THRESHOLD;
      const roster = (payload?.botSeats as SeatIndex[] | undefined) ?? [];
      const levels = (payload?.botLevels as BotLevel[] | undefined) ?? [];
      playerCountRef.current = playerCount;
      eliminationThresholdRef.current = threshold;
      ...
      setGameState(startGame(playerCount, seed, threshold));
      setFinalResult(null);
      setPhase("playing");
    });
    ...
    // online game here (see SplendorGame.tsx/NoThanksGame.tsx).
    ...
    channel.on("broadcast", { event: "state-sync" }, ({ payload }) => {
      const state = payload?.state as FiveCucumbersState | undefined;
      if (!state) return;
      const roster = (payload?.botSeats as SeatIndex[] | undefined) ?? [];
      const levels = (payload?.botLevels as BotLevel[] | undefined) ?? [];
  ...
  const knownEliminationThreshold = host?.eliminationThreshold ?? eliminationThreshold;
  ...
  const sendGameStart = useCallback(() => {
    ...
    channelRef.current?.send({
      ...
      payload: {
        ...
        eliminationThreshold: eliminationThresholdRef.current,
        botSeats: botSeatsRef.current,
        botLevels: botLevelsRef.current,
      },
    });
  ...
  useBotAutoplay<FiveCucumbersState, EngineAction, SeatIndex>({
    active: isHost && phase === "playing",
    state: gameState,
    currentActor: fiveCucumbersCurrentActor,
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      if (!state) return;
      const actor = fiveCucumbersCurrentActor(state);
