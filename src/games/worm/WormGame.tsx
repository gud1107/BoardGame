...
export default function WormGame({ onComplete }: PlayableGameProps) {
        ...
        {intent === "join" && (
          <label className="flex flex-col gap-1.5 text-sm text-white/70 light:text-slate-600">
            ...
            <input
              ...
              placeholder="0000"
              inputMode="numeric"
              className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-center text-lg font-semibold tracking-[0.3em] text-white placeholder:text-white/20 focus:border-lime-400 focus:outline-none light:border-slate-300 light:bg-white light:text-slate-900 light:placeholder:text-slate-400"
            />
          </label>
        )}
        {intent === "create" && (
  ...
  if (phase === "post-game" && finalRankings) {
    // TODO(theme): hardcoded dark inline gradient background — not yet theme-aware,
    // stays dark in light mode. Left untouched (out of scope for CSS/class-only
    // light: pass); the light: overrides below are skipped for text/border inside
    // this panel accordingly, since the surrounding background never changes.
    return withGuard(
      <div
