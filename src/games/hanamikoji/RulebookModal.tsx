      ...
      <div className="flex flex-col gap-5 text-sm text-white/80 light:text-slate-700">
        <section>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {ACTIONS.map((a) => (
              <div key={a.name} className="rounded-xl border border-white/10 bg-white/5 p-3 light:border-slate-200 light:bg-slate-50">
                <p className="mb-1 font-medium text-white light:text-slate-900">
                  {a.emoji} {a.name}
                </p>
                <p className="text-xs text-white/60 light:text-slate-600">{a.desc}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section>
          ...
          <p className="text-white/70 light:text-slate-600">
            게이샤 7명 중 <span className="text-rose-300 light:text-rose-700">4명 이상</span>을 확보하거나, 확보한
            게이샤의 호감도 합이 <span className="text-rose-300 light:text-rose-700">11점 이상</span>이 되는 즉시
