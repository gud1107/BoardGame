 ...
 * action whose `action` is specifically "coup" — the single most dramatic
 * action type (always forces a target to lose an influence, no challenge/
 * block window at all — see engine.ts `needsDeclareTarget`/§3-A), chosen as
 * this game's one system-log headline per the chat pilot's per-game
 * convention (see PerudoGame.tsx's `raise`/DalmutiGame.tsx's `returnTax`).
 */
function formatCoupCoupLog(name: string, targetName: string): string {
  return `${name}님이 ${targetName}님에게 쿠데타를 선언했습니다`;
}

/**
 * Whose decision `useBotAutoplay` should drive right now. Several phases
 * (`actionChallengeWindow`/`blockChallengeWindow`, and 외화 도입's blockWindow
 * open to every other alive seat) can have more than one simultaneously
 * eligible responder — same "lowest undecided seat" workaround every other
 * simultaneous-decision phase in this project uses, built on the engine's
 * own `currentResponders` (also used for UI response gating).
 */
function coupCurrentActor(state: CoupState): SeatIndex | null {
...
export default function CoupGame({ onComplete }: PlayableGameProps) {
  ...
  useEffect(() => {
    ...
    const chatChannel = `room:coup:${roomCode}`;
    ...
    channel.on("broadcast", { event: "game-action" }, ({ payload }) => {
      ...
      if (action.type === "declareAction" && action.action === "coup" && action.targetSeat !== undefined) {
        setChatMessages((prev) => [
          ...prev,
          {
            ...
            deviceId: "system",
            senderName: "시스템",
            body: formatCoupCoupLog(namesRef.current[action.seat] ?? "상대", namesRef.current[action.targetSeat!] ?? "상대"),
            type: "SYSTEM",
  ...
  const chooseAction = useCallback((state: CoupState, actor: SeatIndex): EngineAction | null => {
  ...
  useBotAutoplay<CoupState, EngineAction, SeatIndex>({
    active: isHost && phase === "playing",
    state: gameState,
    currentActor: coupCurrentActor,
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      if (!state) return;
      const actor = coupCurrentActor(state);
  ...
  if (phase === "playing" && gameState && mySeat !== null) {
    ...
    return withGuard(
      ...
      <CoupBoard state={gameState} viewerSeat={mySeat} names={names} connectedSeats={connectedSeats} onAction={handleAction} onGameEnd={handleGameEnd} />
  ...
  if (phase === "post-game" && finalResult) {
    return withGuard(
      ...
      <div className="flex flex-col items-center gap-5 rounded-2xl border border-white/10 bg-white/[0.03] p-8 text-center light:bg-white/90 light:border-slate-200 light:shadow-sm">
        ...
        <p className="text-white/80 light:text-slate-700">{finalResult.winnerName} 님이 최후의 생존자가 되어 게임이 끝났어요.</p>
        <div className="flex gap-2">
