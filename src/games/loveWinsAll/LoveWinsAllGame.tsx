...
export default function LoveWinsAllGame({ onComplete }: PlayableGameProps) {
  ...
  // roles, applies to both the "base" and "lwa2" variant rulesets), so
  // `maxPlayers` is a literal 2 — no numeric target-count variable.
  useActiveRoomListing({
    gameId: "love-wins-all",
    roomCode,
    isHost,
  ...
  if (phase === "choose") {
    return withGuard(
      <RulebookGate
        gameId="love-wins-all"
        containerClassName="border-pink-500/20 bg-gradient-to-b from-[#1a0510] via-[#0d0610] to-black light:border-pink-300 light:bg-none light:bg-white"
        icon="💕"
        title="러브 윈즈 올 온라인 대전"
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 rounded-xl border px-3 py-2.5 text-xs font-semibold transition ${
                  chosenVariant === "base" ? "border-pink-400 bg-pink-500/15 text-pink-100" : "border-white/15 light:border-slate-200 text-white/60 light:text-slate-600 hover:border-white/30"
                }`}
              >
                기본판
                <span className="mt-0.5 block break-keep text-[10px] font-normal text-white/40 light:text-slate-500">30장 · 3장 손패 · 칩 25개</span>
              </button>
              <button
                type="button"
                onClick={() => setChosenVariant("lwa2")}
                className={`flex-1 rounded-xl border px-3 py-2.5 text-xs font-semibold transition ${
                  chosenVariant === "lwa2" ? "border-pink-400 bg-pink-500/15 text-pink-100" : "border-white/15 light:border-slate-200 text-white/60 light:text-slate-600 hover:border-white/30"
                }`}
              >
                러브 윈즈 올 2
                <span className="mt-0.5 block break-keep text-[10px] font-normal text-white/40 light:text-slate-500">49장 · 커뮤니티+라이어 · 칩 35개</span>
              </button>
            </div>
          </div>
        )}
        {intent === "join" && (
          <label className="flex flex-col gap-1.5 text-sm text-white/70 light:text-slate-700">
            ...
            <input
              ...
              placeholder="0000"
              inputMode="numeric"
              className="rounded-lg border border-white/10 light:border-slate-200 bg-white/5 light:bg-slate-900/5 px-3 py-2 text-center text-lg font-semibold tracking-[0.3em] text-white light:text-slate-900 placeholder:text-white/20 focus:border-pink-400 focus:outline-none"
            />
          </label>
        )}
          ...
          {phase === "connecting" ? (
            ...
            <>
              ...
              <button
                ...
                className="rounded-full border border-white/15 light:border-slate-200 px-4 py-2 text-xs text-white/70 light:text-slate-700 hover:border-white/30"
              >
                🔗 초대 링크 복사
              </button>
    ...
    return withGuard(
      <>
        <div className="flex flex-col items-center gap-5 rounded-2xl border border-pink-500/20 bg-gradient-to-b from-[#1a0510] via-[#0d0610] to-black p-8 text-center">
          ...
          <p className="text-white/80 light:text-slate-700">{headline}</p>
          <div className="flex gap-2">
