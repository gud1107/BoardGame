...
import ForSaleBoard from "./ForSaleBoard";
import { formatDollars } from "./CardArt";
 ...
 * "지수님이 $3,000에 입찰했습니다".
 */
function formatForSaleBidLog(name: string, amount: number): string {
  return `${name}님이 ${formatDollars(amount)}에 입찰했습니다`;
}

/**
 * Whose decision `useBotAutoplay` should drive right now. Phase 1 (buying)
 * has a strict turn order (`auction.activeSeat`). Phase 2 (selling) is
 * simultaneous blind submission by rule — every seat decides independently
 * — but `useBotAutoplay` models a single pending actor, so this returns the
 * lowest-numbered seat that hasn't submitted yet; bots occupying a later
 * seat than an un-submitted human end up waiting for that human to submit
 * first (documented simplification — harmless since a full-bot lobby has no
 * human to wait on, which is what the automated tests exercise).
 */
function forSaleCurrentActor(state: ForSaleState): SeatIndex | null {
  if (state.phase === "buying" && state.auction) return state.auction.activeSeat;
  if (state.phase === "selling" && state.sale && !state.sale.revealed) {
    for (let seat = 0; seat < state.playerCount; seat++) {
      if (state.sale.submissions[seat] === undefined) return seat;
    }
  }
  return null;
}
 ...
 * itself modeled on dalmuti/DalmutiGame.tsx): every connected client
 * independently computes the full `ForSaleState` (every seat's cash/hand,
 * every Phase-2 submission) from a shared RNG seed plus replayed
 ...
 * the accepted trust trade-off; cash/hand secrecy stays enforced at the UI
 * layer only (`ForSaleBoard.tsx`). No house-rule toggle here — the rulebook
 * has no optional variant to expose.
 */
...
function getStoredSeat(code: string): number | null {
  const v = window.localStorage.getItem(`for-sale-seat-${code}`);
...
function storeSeat(code: string, seat: number) {
  window.localStorage.setItem(`for-sale-seat-${code}`, String(seat));
}
...
export default function ForSaleGame({ onComplete }: PlayableGameProps) {
  ...
  const [gameState, setGameState] = useState<ForSaleState | null>(null);
  ...
  const gameStateRef = useRef<ForSaleState | null>(null);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`for-sale-room-${roomCode}`, {
      config: { broadcast: { self: true }, presence: { key: deviceId } },
    });
    channelRef.current = channel;
    startSentRef.current = false;

    const chatChannel = `room:for-sale:${roomCode}`;
    ...
    channel.on("broadcast", { event: "game-action" }, ({ payload }) => {
      ...
      // — no server round-trip, no change to the pure reducer in engine.ts.
      // Deliberately not persisted to `chat_messages` (unlike user
      // messages) — every client would otherwise write a duplicate row, and
      // it's trivially re-derivable from the replayed action log anyway.
      if (action.type === "bid") {
        setChatMessages((prev) => [
          ...prev,
          {
            ...
            deviceId: "system",
            senderName: "시스템",
            body: formatForSaleBidLog(namesRef.current[action.seat] ?? "상대", action.amount),
            type: "SYSTEM",
    ...
    channel.on("broadcast", { event: "state-sync" }, ({ payload }) => {
      const state = payload?.state as ForSaleState | undefined;
      if (!state) return;
      const roster = (payload?.botSeats as SeatIndex[] | undefined) ?? [];
      const levels = (payload?.botLevels as BotLevel[] | undefined) ?? [];
  ...
  const sendChatMessage = useCallback(
    (rawBody: string): SendResult => {
      ...
      const message: ChatMessage = {
        id: uuid(),
        channel: `room:for-sale:${roomCode}`,
        deviceId,
  ...
  const chooseAction = useCallback((state: ForSaleState, actor: SeatIndex): EngineAction | null => {
  ...
  useBotAutoplay<ForSaleState, EngineAction, SeatIndex>({
    active: isHost && phase === "playing",
    state: gameState,
    currentActor: forSaleCurrentActor,
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      if (!state) return;
      const actor = forSaleCurrentActor(state);
    ...
    return withGuard(
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-amber-400/30 bg-amber-400/10 light:border-amber-300 light:bg-amber-50 p-8 text-center">
        ...
        <p className="max-w-sm text-sm text-amber-100/80 light:text-amber-800">
          포세일은 실시간 온라인 대전 전용이라 Supabase 설정이 필요합니다.
  ...
  if (phase === "room-full") {
    return withGuard(
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-rose-400/30 bg-rose-400/10 light:border-rose-300 light:bg-rose-50 p-8 text-center">
        ...
        <button onClick={handleLeave} className="mt-2 rounded-full bg-sky-600 px-5 py-2 text-sm font-semibold text-white hover:bg-sky-500">
          처음으로
        </button>
      </div>
    );
      ...
      <RulebookGate
        ...
        description={
          <p className="text-sm text-white/50 light:text-slate-500">
            {MIN_PLAYERS}~{MAX_PLAYERS}명이 각자 기기로 접속해서 실시간으로 플레이해요. 부동산을 낙찰받고, 비싸게 파세요!
          </p>
        }
        actions={
    ...
    return withGuard(
      <div className="flex flex-col gap-4 rounded-2xl border border-white/10 bg-white/[0.03] light:border-slate-200 light:bg-white/90 light:shadow-sm p-6">
        ...
        {intent === "join" && (
          <label className="flex flex-col gap-1.5 text-sm text-white/70 light:text-slate-600">
            ...
            <input
              ...
              placeholder="0000"
              inputMode="numeric"
              className="rounded-lg border border-white/10 bg-white/5 light:border-slate-300 light:bg-white px-3 py-2 text-center text-lg font-semibold tracking-[0.3em] text-white light:text-slate-900 placeholder:text-white/20 light:placeholder:text-slate-400 focus:border-sky-400 focus:outline-none"
            />
          </label>
        )}
        {intent === "create" && (
        ...
        <div className="flex gap-2">
          ...
          <button onClick={enterRoom} className="flex-1 rounded-xl bg-sky-600 py-2.5 text-sm font-semibold text-white hover:bg-sky-500">
        ...
        {phase === "connecting" ? (
          ...
          <>
            ...
            {isHost && occupants.length + botSeats.length >= MIN_PLAYERS && occupants.length + botSeats.length < knownTargetPlayerCount && (
              <button onClick={sendGameStart} className="rounded-full bg-sky-600 px-4 py-2 text-xs font-semibold text-white hover:bg-sky-500">
  ...
  if (phase === "playing" && gameState && mySeat !== null) {
    ...
    return withGuard(
      ...
      <ForSaleBoard state={gameState} viewerSeat={mySeat} names={names} connectedSeats={connectedSeats} onAction={handleAction} onGameEnd={handleGameEnd} />
  ...
  if (phase === "post-game" && finalResult) {
    return withGuard(
      ...
      <div className="flex flex-col items-center gap-5 rounded-2xl border border-white/10 bg-white/[0.03] light:border-slate-200 light:bg-white/90 light:shadow-sm p-8 text-center">
        ...
        <p className="text-white/80 light:text-slate-700">{finalResult.winnerNames} 님이 최고 부동산 중개인이 되어 게임이 끝났어요.</p>
        <div className="flex gap-2">
          ...
          <button onClick={handleRematch} className="rounded-xl bg-sky-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-sky-500">
            다시하기
          </button>
        </div>
      </div>
